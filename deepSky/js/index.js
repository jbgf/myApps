  ...
  !function() {
    function t(e, n) {
      ...
      "function" != typeof this.nextFps && (t.prototype.nextFps = function() {
        ...
        this.speed += f.dSpeed, 
        this.len += .7, 
        Math.abs(this.point.y) > h / 2 * .7 
        ? this.lWidth = 5 
        : Math.abs(this.point.y) > h / 2 * .5 
          ? this.lWidth = 4 
          : Math.abs(this.point.y) > h / 2 * .3 
            ? this.lWidth = 3 
            : Math.abs(this.point.y) > h / 2 * .2 && (this.lWidth = 2), 

        this.opc = this.opc < .1 
                   ? .1 
                   : this.opc - .02, 

        (Math.abs(this.point.x) > r / 2 || Math.abs(this.point.y) > h / 2) 
        && (this.isEnd = !0)
      }), 
    ...
    function e(t) {
      ...
      "function" != typeof this.nextFps && (e.prototype.nextFps = function() {
        if (1 == v.way) {
          if (
            this.point.x = 
              (Math.abs(this.point.x) 
                + this.speed * (Math.abs(this.point.x) 
                                  / (Math.abs(this.point.x) + Math.abs(this.point.y))
                                )
              ) 
              * (this.point.x / Math.abs(this.point.x)), 

            this.point.y = 
              (Math.abs(this.point.y) 
                + this.speed * (Math.abs(this.point.y) 
                                  / (Math.abs(this.point.x) + Math.abs(this.point.y))
                                )
              ) 
              * (this.point.y / Math.abs(this.point.y)), 
          ...
          var e = (this.R - Math.abs(this.point.x)) / 
              this.R * (this.speed + 1 + 3 * Math.random());
          e = e <= .05 ? .05 : e, 
          e = e >= 1 ? 1 : e, 
    ...
    function n(t) {
        ...
        "function" != typeof this.nextFps && (
            n.prototype.nextFps = function() {
                this.speed += 2, this.rad += .3, 
                this.point.x = (
                  Math.abs(this.point.x) + 
                  this.speed 
                  * (Math.abs(this.point.x) 
    ...
    function i() {
      ...
      "function" != typeof this.nextFps && (
        i.prototype.nextFps = function() {
          ...
          ? this.step += .01 
          : this.step < 1 
            && (this.step += .006, this.opc > .01 && (this.opc -= .01)), 
      ...
      var p, u = {
        ...
        f = {
          ...
          newPoint: function(t, e) {
            var n = t.x * t.x + t.y * t.y + e * e 
                    + 2 * e * Math.sqrt(t.x * t.x + t.y * t.y);
            return {
        ...
        _ = function() {
          ...
          case 2:
            g.drawParticle(), 
            g.drawParticle2(), 
            f.createRay(Math.ceil(Math.random() * f.avgMax)), g.drawRay(), w.nextFps(), w.isEnd || w.draw(d);
            break;
          case 3:
            g.drawParticle(), 
            g.drawRay();
