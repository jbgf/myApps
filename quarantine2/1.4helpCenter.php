<?php include './common/centerhead.html' ?>
<link rel="stylesheet" type="text/css" href="css/home.css">
<link rel="stylesheet" type="text/css" href="css/font-awesome.css">

<link rel="stylesheet" type="text/css" href="iconfont/iconfont.css">

<script type="text/javascript" src="js/home.js"></script>
<script type="text/javascript" src="js/jquery.validate.js"></script>


<!-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
      
    <div class="banner middleTable" style="background:url(img/banner04.jpg) center center">
        <div class="middleRow">
            <div class="banner_inner">
                <div class="introduceBox">
                    <div class="bigh white">
                        企业中心
                    </div> 
                    <div class="e white">
                        Enterprise Center
                    </div> 
                </div>
            </div>    
        </div>
    </div>
    <div class="centerMiddle container1000">
        <div class="leftSide">

            <div class="sideBar02">
                <ul class="sideBar_ul">
                    <li class="sideBar_li ">
                        <a class="sideBar_a">
                            <p>
                            <span class="iconBg"><i class="iconfont ">&#xe6ae;</i></span>
                            <span class="icontext">企业信息</span>
                            </p>
                        </a>
                        <ul class="sideBar_ul hide" style="">
                            <li class="sideBar_li">
                                <a class="sideBar_a">
                                    <p>
                                        <span class="icontext">- 修改密码</span>
                                    </p>
                                </a>
                            </li>
                            <li class="sideBar_li ">
                                <a class="sideBar_a">
                                    <p>
                                        <span class="icontext">- 修改账户信息</span>
                                    </p>
                                </a>    
                            </li>
                        </ul>
                    </li>
                    
                    <li class="sideBar_li ">
                        <a class="sideBar_a">
                            <p>
                                <span class="iconBg"><i class="iconfont ">&#xe601;</i></span>
                                <span class="icontext">委托单信息</span>
                            </p>
                        </a>
                        
                    </li>
                    
                    <li class="sideBar_li">
                        <a class="sideBar_a">
                            <p>
                                <span class="iconBg"><i class="iconfont ">&#xe66c;</i></span>
                                <span class="icontext">网上委托</span>
                            </p>
                        </a>
                    </li>
                    <li class="sideBar_li">
                        <a class="sideBar_a">
                            <p>
                                <span class="iconBg"><i class="iconfont ">&#xebc8;</i></span>
                                <span class="icontext">帮助中心</span>
                            </p>
                        </a>
                    </li>
                </ul>
                <div class="ulAddon"></div>
                <script type="text/javascript">
                    $(function(){

                        $(".sideBar02 .sideBar_li").each(function(i,e){
                            var childList = $(e).children("ul.sideBar_ul");
                                childList.length>0 ? 
                                $(e).addClass("childList")
                                    .on("click",function(){
                                                childList.slideToggle();
                                                $(this).toggleClass("on")
                                    })
                            : $(e).children(".sideBar_a").addClass("bottom");
                        })
        
                    })
                    
                </script>
            </div>
        </div> 
        <div class="rightWrapper">
            <div class="header02 clear">
              <div class="more float-l">你的位置：<a href="/">网站首页</a> &gt; <a href="/index.php?s=news&amp;c=category&amp;id=1">产品检测范围</a>&gt;<a href="/index.php?s=news&amp;c=category&amp;id=36">食品理化检测</a>                              
              </div>
            </div>
            <div class="commonWrapper">
                 <p class="chead">帮助中心</p>
                 <div class="hcWrapper">
                     <div class="tsection">
                         <div class="toggleBar">如何进行网上委托? <i class="fa fa-angle-up" aria-hidden="true"></i></div>
                         <div class="t">
                             1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?4、行网上委托?如何进行网上委托?5、委托?如何进行网上委托?6、何进行网上委托?如何进行网上委托?7、如何进行网上委托?如何进行网上委托?1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托? <a class="blue more" >...更多</a>  
                         </div>
                     </div>
                     <div class="tsection">
                         <div class="toggleBar">如何进行网上委托? <i class="fa fa-angle-up" aria-hidden="true"></i></div>
                         <div class="t">
                             1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?4、行网上委托?如何进行网上委托?5、委托?如何进行网上委托?6、何进行网上委托?如何进行网上委托?7、如何进行网上委托?如何进行网上委托?1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?   
                         </div>
                     </div> 
                     <div class="tsection">
                         <div class="toggleBar">如何进行网上委托? <i class="fa fa-angle-up" aria-hidden="true"></i></div>
                         <div class="t">
                             1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?4、行网上委托?如何进行网上委托?5、委托?如何进行网上委托?6、何进行网上委托?如何进行网上委托?7、如何进行网上委托?如何进行网上委托?1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?   
                         </div>
                     </div>
                     <div class="tsection">
                         <div class="toggleBar">如何进行网上委托? <i class="fa fa-angle-up" aria-hidden="true"></i></div>
                         <div class="t">
                             1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?4、行网上委托?如何进行网上委托?5、委托?如何进行网上委托?6、何进行网上委托?如何进行网上委托?7、如何进行网上委托?如何进行网上委托?1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?   
                         </div>
                     </div>
                     <div class="tsection">
                         <div class="toggleBar">如何进行网上委托? <i class="fa fa-angle-up" aria-hidden="true"></i></div>
                         <div class="t">
                             1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?4、行网上委托?如何进行网上委托?5、委托?如何进行网上委托?6、何进行网上委托?如何进行网上委托?7、如何进行网上委托?如何进行网上委托?1、如何进行网上委托?如何进行网上委托?2、进行网上委托?如何进行网上委托? 如何进行网上委托?如何进行网上委托?3、委托?如何进行网上委托?   
                         </div>
                     </div>
                     <script type="text/javascript">
                         var last;
                         $(".hcWrapper .toggleBar").on("click",function(){
                             if($(this).hasClass("on")){
                                last = undefined;
                                $(this).removeClass("on").next(".t").slideToggle();
                                return
                             }
                             if(last){last.removeClass("on").next(".t").slideToggle()}
                             $(this).addClass("on").next(".t").slideToggle()
                             last = $(this);
                         })
                     </script> 
                 </div> 
                 <div class="pagination03 align-center">
                                        
                    <ul>
                        <li class="left"><a>上一页</a></li>
                        <li class=""><a>1</a></li>
                        <li class=""><a>2</a></li>
                        <li class="on"><a>3</a></li>
                        <li class=""><a>4</a></li>
                        <li class=""><a>5</a></li>
                        <li class="ellipsis"><a>···</a></li>
                        <li class="right"><a>下一页</a></li>

                    </ul>
                 </div>   
            </div>
        </div>  
    </div>
      
       
<!-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->  
    
<?php include './common/centerfoot.html' ?>    