<?php include 'head.html' ?>
    <div class="innerbanner" style="background:url(img/banner04.png) center center ">
        <div class="container1200"><div class="iText">search results</div></div>
 
    </div>
    <div class="centerMiddle container1200">
        <div class="center_innerWrapper">
            
            <div class="search_wrapper">
                <div class="news_h02">
                    <div class="h-title01 float-l">
                        <span class="text"> search results </span>

                    </div>
                    <div class="tips">We found  <span class="blue">5</span> reviews</div>
                    <span class="r-link float-r">
                        <a class="link"> Home</a>
                        
                        <a class="link on">search results</a>
                    </span>
                      
                </div>
                  
                <div class="contentPart">
                    <div class="news_wrapper">
                        
                          
                              <div id="slideBd2" class="bd">
                                <ul>
                                  <li class="">
                                    <a href="#">
                                    <div class="slideBd2_left ">
                                      <img class="dataImg full" src="img/icon3.png">
                                      <div class="date">
                                          <span class="day">01</span><br>
                                          <span class="ym">2017-Feb</span>
                                      </div>
                                    </div>
                                    <div class="slideBd2_right">
                                      <div class="title">
                                        <span class="ellipsis">Delegation from STC visited FQII</span>
                                      </div>
                                      <div class="news_intro">January 10th, 2017, Mr.Li Jinquan, the General Manager of Hong.</div>
                                      <div class="arrow"></div>
                                    </div>
                                    </a>
                                  </li>
                                  <li class="">
                                    <a href="#">
                                    <div class="slideBd2_left ">
                                      <img class="dataImg full" src="img/icon3.png">
                                      <div class="date">
                                          <span class="day">01</span><br>
                                          <span class="ym">2017-Feb</span>
                                      </div>
                                    </div>
                                    <div class="slideBd2_right">
                                      <div class="title">
                                        <span class="ellipsis">Delegation from STC visited FQII</span>
                                      </div>
                                      <div class="news_intro">January 10th, 2017, Mr.Li Jinquan, the General Manager of Hong.</div>
                                      <div class="arrow"></div>
                                    </div>
                                    </a>
                                  </li>
                                  <li class="">
                                    <a href="#">
                                    <div class="slideBd2_left ">
                                      <img class="dataImg full" src="img/icon3.png">
                                      <div class="date">
                                          <span class="day">01</span><br>
                                          <span class="ym">2017-Feb</span>
                                      </div>
                                    </div>
                                    <div class="slideBd2_right">
                                      <div class="title">
                                        <span class="ellipsis">Delegation from STC visited FQII</span>
                                      </div>
                                      <div class="news_intro">January 10th, 2017, Mr.Li Jinquan, the General Manager of Hong.</div>
                                      <div class="arrow"></div>
                                    </div>
                                    </a>
                                  </li>
                                  <li class="">
                                    <a href="#">
                                    <div class="slideBd2_left ">
                                      <img class="dataImg full" src="img/icon3.png">
                                      <div class="date">
                                          <span class="day">01</span><br>
                                          <span class="ym">2017-Feb</span>
                                      </div>
                                    </div>
                                    <div class="slideBd2_right">
                                      <div class="title">
                                        <span class="ellipsis">Delegation from STC visited FQII</span>
                                      </div>
                                      <div class="news_intro">January 10th, 2017, Mr.Li Jinquan, the General Manager of Hong.</div>
                                      <div class="arrow"></div>
                                    </div>
                                    </a>
                                  </li>
                                  
                                </ul>
                              </div>
                              
                              <div class="pagination02 align-center" >
                                        
                                <ul>
                                    <li class="left"><a>Previous</a></li>
                                    <li class=""><a>1</a></li>
                                    <li class=""><a>2</a></li>
                                    <li class="on"><a>3</a></li>
                                    <li class=""><a>4</a></li>
                                    <li class=""><a>5</a></li>
                                    <li class="ellipsis"><a>···</a></li>
                                   
                                    <li class="right"><a>Next</a></li>

                                </ul>
                              </div>
                          
                          
                       
                    </div> 
                </div>
            </div> 
        </div>
    </div>
   


<?php include 'foot.html' ?>