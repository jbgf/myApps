...
document.write('<script src="js/jquery.artDialog.js?skin=opera"></script><script src="js/iframeTools.js"></script>');
//===========================Ajax标签结束开始===============================================
...
//检查是否手机号
function ismobile(s){
   var p = /^(013|015|13|14|15|018|18)\d{9}$/;
   if(s.match(p) != null){
  return true;
  }
  return false;
...
{   var el=event.srcElement;
	...
	else
	{
		...
		{
			var em=obj.children[i];
			for(var j=0;j<em.children.length;j++)
			{
				//if (em.children[j].tagName=='INPUT')
				if (em.children[j].type=='checkbox'){
					cobj=em.children[j];
					if (cobj.checked==''){
						 cobj.checked='checked';
						 obj.style.background=selectedcolor;
					 } else {
					   cobj.checked='';
...
function showuserlogin(){
	$.dialog.open(appurl+"user/userlogin.aspx",{id:"loginbox",lock:true,fixed:true,title:"<div style='text-align:left'><img src='"+appurl+"sysimg/default/user.png' align='absmiddle'>用户登录</div>",width:320,height:380,max:false,min:false});
}
//检查是不是数字
isNumber = function (e) {   
    if ($.browser.msie) {   
