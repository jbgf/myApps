if (typeof HdGame == "undefined") { HdGame = {} }(function(a) { a.isUrl = function(f, e) {
    ...
    a.checkTextLength = function(o, f, k) {
        ...
        for (var e = 0; e < j.length; e++) {
            ...
            if (c == "*") { h++ } }
        var n = 0;
        if (h % 2 == 0) { n = h / 2 + (o * 1 - h);
            l = g.substr(0, n) } else { n = (h - 1) / 2 + (o * 1 - h);
            l = g.substr(0, n) }
        if (k) {
            return { isOverflow: true, returnValue: l } } else { f.val(l) } };
    a.fadOut = function(c, d) {
        ...
        setTimeout(function() { c.removeClass("poupFadIn");
            c.css("opacity", 1);
            d && d() }, 200) };
    ...
    a.statusMsg = function(d, c, e) {
        if ((d == 4 || (d == 6 && g_config.style != 50)) && g_config.showHelpGuide) { $("#helpGuideBox").show();
            return }
        if ((d == 7 || d == 8) && g_config.showHelpGuide) { $("#helpGuideBox").show();
            return }
        var g = "",
            f;
        if (typeof d === "string") { g = d } else {
            if (d === 1) { g = "活动尚未开始";
                f = "开始时间为" + g_config.startTime } else {
                if (d === 3) { g = "活动已结束";
                    f = "请关注期待下次活动" } else {
                    if (d === 4) { g = "您今天已经没有抽奖机会了";
                        f = "明天可继续抽奖哦";
                        if (!e) { e = "14rem" } } else {
                        if (d === 5) { g = "未授权用户无法进行游戏。" } else {
                            if (d === 6) { g = "您的抽奖机会已经用完" } else {
                                if (d === 7) { g = "您今天已经没有参与机会了";
                                    f = "明天可继续参与哦" } else {
                                    if (d === 8) { g = "您的参与机会已经用完";
                                        f = "详情留意活动说明" } } } } } } } }
        if (f && typeof c !== "string") { c = f }
        a.showMsg(g, c, e) };
    a.showMsg = (function() {
        var e = null;
        ...
        var f = c.find(".sTime");
        var h = false;
        return function(k, i, j) { d.html(k);
            if (typeof i === "string") { f.text(i) } else {
                if (i) { f.hide() } else { f.show() } }
            if (j) { c.css("width", j) } else { c.css("width", "11.5rem") }
            if (!h) { $("body").append(c);
                h = true }
            clearTimeout(e);
            a.fadOut(c);
            e = setTimeout(function() { a.fadIn(c, function() { c.remove();
                    h = false }) }, 2000) } })();
    a.showMsgToast = function(f, d, e) {
        var c = $("<div class='msg-toast " + e + "' style='display: none;'>" + f + "</div>");
        window.top.$("body").append(c);
        c.fadeIn(300);
        setTimeout(function() { c.fadeOut(300, function() { c.remove() }) }, d) };
    a.unPublishMsg = function() {
        var c = '<div class="unPublish">活动尚未发布，当前仅供预览</div>';
        $("body").prepend(c) };
    a.share = function() {
        if ($("#sharePanel").length > 0) { $("#sharePanel").show();
            return }
        $("body").append("<div id='sharePanel'></div>");
        $("#sharePanel").on("touchend touchstart", function() { $(this).hide() }) };
    a.favorite = function() { a.logDog(1000045, 2);
        if ((g_config.wxAward && g_config.wxAward.genewxcard)) {
            return }
        if ($("#favoritePanel").length > 0) { $("#favoritePanel").show();
            return }
        $("body").append("<div id='favoritePanel'></div>");
        $("#favoritePanel").on("touchend touchstart", function(c) { setTimeout(function() { $("#favoritePanel").hide() }, 500) }) };
    ...
    (function() {
        ...
        var d = function(j, l, m) {
            ...
            if (typeof l === "object" && typeof l.x !== "undefined" && typeof l.y !== "undefined" && typeof l.w !== "undefined" && typeof l.h !== "undefined") {
                ...
                g.children().css({ width: k, height: k });
                g.css({ left: l.x + (l.w - k) / 2, top: l.y + (l.h - k) / 2 }) } else { m = m || 0.8;
                var f = j.outerHeight();
                var h = j.outerWidth();
                var i = (l === "w" ? h / 2 : (l === "h" ? f / 2 : (l === "min" ? Math.min(f, h) / 2 : Math.max(f, h) / 2))) * m;
                g.css({ left: h / 2 - i, top: f / 2 - i });
                g.children().css({ width: 2 * i, height: 2 * i }) }
            j.append(g);
            e = g;
            j.data("_guide", g);
            _lastWrap = j };
        var c = { play: d, stop: _stop };
        a.Guide = c })();
    a.homePoup = function(c) {
        if (c == 1) { $("#rankBox").show() } else {
            if (c == 3) { $("#awardBox").show() } else {
                if (c == 4) { $("#regAwardBox").show() } } }
        $("#poupInfoBox").show() };
    a.poupAjaxComplete = function(c) {
        var d = $(".ajaxLoadBar");
        d.addClass("ajaxComplete");
        $(".ajaxLoadBg").hide();
        setTimeout(function() { d.removeClass("ajaxLoad");
            d.removeClass("ajaxComplete") }, 500) };
    (function() {
        var p = { rankUrl: "", awardUrl: "", getRegAwardUrl: "", mhaveScore: 0, openId: "", gameId: 0, };
        var f = { menuLen: "", slideBarWidth: "", slideBarMaxWidth: "", marginLeft: false, firstTouchRank: true, firstTouchAward: true, firstTouchWinList: true, hasInitEvent: false, };
        var o = { isLoad: false, isRankAll: false, start: 0, num: 0, limit: 99 };
        a.initChangePoup = function(q) { q && $.extend(p, q);
            if (_manage) { $("#Award_Round_Dot").show() }
            f.menuLen = 0;
            $(".poupTitleBox .poupTitleMune").each(function() {
                if ($(this).css("display") == "block") { $(this).removeClass("hide");
                    f.menuLen++ } else { $(this).addClass("hide") } });
            ...
            $(".poupTitleBox .poupTitleMune").not(".hide").each(function(v, x) {
                ...
                for (var w = 0; w < $("#poupInfoBox .poupMain").length; w++) {
                    ...
                    if (t.attr("_flag") == u) { t.show();
                        if (r) { r = r.add(t) } else { r = t }
                        return } } });
            ...
            if (f.slideBarWidth > f.slideBarMaxWidth) { f.marginLeft = true }
        ...
        function l() {
            ...
            $("#rankBox").scroll(function(s) {
                ...
                if ($("#rankHeight").height() - $("#rankHeight").height() / 3 <= $(window).height() + $("#rankBox").scrollTop()) {
                    if (!o.isLoad && !o.isRankAll) { o.isLoad = true;
                        i() } } }) }
        a.changePoup = function(r, s) { o.isClose = false;
            $("#poupInfoBox").show();
            $(".poupTitleMune").removeClass("checked");
            if (r == 5 && p.hasReport == true) { $("#informResultBox h3").text("你已经投诉过了！");
                $("#informResultBox").show() } else {
                if (typeof s === "number") {
                    var t = $(".poupTitleMune").filter(":visible").eq(s);
                    t.addClass("checked");
                    r = parseInt(t.attr("_flag")) } else { $(".poupTitleBox .poupTitleMune").filter(":visible").each(function(v, w) {
                        if ($.trim($(this).attr("_flag")) == r) { s = v;
                            $(this).addClass("checked") } }) } }
            if (_manage) {
                var u = $.trim($(".poupTitleMune").eq(s).find(".item").text());
                var q = parent.$("#editActive .topBar .column");
                if (u == "活动说明") { q.each(function() {
                        if ($(this).find(".name").text().trim() == "活动奖品") { $(this).addClass("checked");
                            parent.$(".topBar .transitionPanel").css("left", ($(this).offset().left + 2) + "px") } }) } else { q.each(function() {
                        var v = $.trim($(this).find(".name").text());
                        if (v == u) { parent.$(".topBar .transitionPanel").css("left", ($(this).offset().left + 2) + "px");
                            q.removeClass("checked");
                            $(this).addClass("checked") } }) } }
            if (f.marginLeft) { $(".poupSlideBar .slideBarTip").css("left", ((13.25 / f.menuLen) * s + (f.slideBarWidth - f.slideBarMaxWidth) / 40) + "rem") } else { $(".poupSlideBar .slideBarTip").css("left", (13.25 / f.menuLen) * s + "rem") }
            e(r);
            $(".popTabBox").css("left", -$(window).width() * s) };

        function e(q) { $("#poupInfoBox").removeClass("poupFadOut").addClass("poupFadIn");
            if (!_manage) { $(".gameBox,.home,.body").addClass("overflow-y-hidden");
                if (g_config.style != 55) { g_config.showSkillSup && $(".bottomSkill").hide() } }
            $(".poupClose").off("touchend");
            setTimeout(function() { $(".poupClose").on("touchend", function(s) { s.preventDefault();
                    s.stopPropagation();
                    ...
                    if (!_manage) { $(".gameBox,.home,.body").removeClass("overflow-y-hidden");
                        g_config.showSkillSup && $(".bottomSkill").show() }
                    setTimeout(function() { r.hide();
                        $(".popTabBox").css("left", 0);
                        $(".poupSlideBar .slideBarTip").css("left", (f.slideBarWidth - f.slideBarMaxWidth) / 40 + "rem") }, 100);
                    hg.fire("hidePoup", q) }) }, 300);
            if (q === 1) { d(q) } else {
                if (q === 2) { g(q) } else {
                    if (q === 3) { k(q) } else {
                        if (q === 4) { h(q) } else {
        ...
        function n() {
            ...
            if (q === "正在加载中..." || q === "") { $("#loadingTxt").html("正在加载中.") } else {
                if (q === "正在加载中.") { $("#loadingTxt").html("正在加载中..") } else { $("#loadingTxt").html("正在加载中...") } }
            o.timer = setTimeout(arguments.callee, 300) }

        function m() { $("#rankHeight").find("#loadingTxt").remove();
            clearTimeout(o.timer) }

        function i(q) {
            if ($("#loadingTxt").length == 0) { $("#rankHeight").append('<div id="loadingTxt" class="rankInfo" style="text-align: left; margin: 0 auto; line-height:2.5rem; font-size:0.6rem; width: 3.8rem; ">正在加载中...</div>');
                n() } else {
                return }
            $.ajax({ type: "post", url: p.rankUrl + "&start=" + o.start + "&limit=" + o.limit + "&playerId=" + g_config.playerId, error: function() {
                    if (o.num == 0) { a.poupAjaxComplete(q) }
                    o.isLoad = false }, success: function(t) { a.tlog("creatRankList", t);
                    var u = $.parseJSON(t);
                    var r = u.rankList;
                    var v = u.rank;
                    var s = $("#rankInfoBox");
                    if (u.isRankAll) { o.isRankAll = true }
                    if (o.num == 0) { a.poupAjaxComplete(q) }
                    setTimeout(function() {
                        if (v == 0) { $("#rank").text("无") } else { $("#rank").text(v + "位") }
                        a.logDog(1000010);
                        var w = r.length;
                        if (w == 0) { $("#noRank").show() } else { $("#rankMain").show();
                            for (var y = 0; y < r.length; y++) {
                                if (o.start + y + 1 >= rankShowNum) { o.isRankAll = true }
                        ...
                        hg.fire("updateRankList", u) }, 500);
                    if (o.num == 0) { f.firstTouchRank = false }
                    m() } }) }

        function d(w) {
            if (g_config.isDoubleGame) { $("#rankBox").addClass("isDoubleGame") }
            if (_manage) {
                var x = [{ name: "magazine", achievement: 999 }, { name: "hubert", achievement: 999 }, { name: "lvox", achievement: 999 }, { name: "hth", achievement: 999 }, { name: "monica", achievement: 999 }, { name: "lzz", achievement: 999 }, { name: "william", achievement: 999 }, { name: "sinki", achievement: 999 }, { name: "weiqizhou", achievement: 999 }, { name: "candyq", achievement: 999 }, { name: "jarvis", achievement: 999 }, { name: "johnvi", achievement: 999 }, { name: "tomato", achievement: 999 }, { name: "tina", achievement: 999 }];
                $("#rankMain").show();
                a.homePoup(w);
                var s = $("#rankInfoBox");
                ...
                if (g_config.isDoubleGame) { u = '<div class="userImg manImg"></div><div class="userImg girlImg"></div>' } else { u = '<div class="userImg manImg"></div>' }
                ...
                $("#rankInfoBox tr.rankInfo:gt(" + z + ")").hide() } else { a.logDog(1000010, 0);
                if (f.firstTouchRank) { $(".ajaxLoadBg").show();
        ...
        function g(q) {
            if (!_manage) { a.logDog(1000009);
                if (skillSupportType == 2 || skillSupportType == 3) {
                    if (g_config.authVer == HdVerDef.FREE) { a.logDog(1000069, 1) } else {
                        if (g_config.authVer == HdVerDef.BJ) { a.logDog(1000069, 3) } else {
                            if (g_config.authVer == HdVerDef.BY) { a.logDog(1000069, 5) } } }
                    a.logDog(1000028, 2) } }
            if (isLimitDraw && (drawTimesLimit == drawTotalLimit)) { $("#explainBox").find(".dayFont").hide();
                $("#explainBox").find(".drawTotalFont").hide() }
            if (!isLimitDraw) { $("#explainBox").find(".drawTotalFont").hide() }
            $("#ruleBox").show();
            $("#poupInfoBox").show();
            if (!p.mhaveScore) { $("#haveScore").hide() }
            if (gameType == 3 && g_config.isCheckPlayTimes) { $("#explainPlayInfoBox").show() } else { $("#explainPlayInfoBox").hide() } }

        function k(x) {
            if (!_manage) { a.logDog(1000011, 0) }
            var q = new Array();
            var u = $("#awardInfo");
            ...
            if (!_manage && g_config.showSkillSup) {
                if (g_config.authVer == HdVerDef.FREE) { a.logDog(1000069, 0) } else {
                    if (g_config.authVer == HdVerDef.BJ) { a.logDog(1000069, 2) } else {
                        if (g_config.authVer == HdVerDef.BY) { a.logDog(1000069, 4) } } }
                a.logDog(1000028, 1);
                typeof g_config.isAOpenId == "number" && a.logDog(1000115, 1 + g_config.isAOpenId) }
            if (g_config.aid == 7171826 && g_config.showSkillSup) { a.logDog(1000062, 0) } else {
                if (g_config.aid == 8566249 && g_config.showSkillSup) { a.logDog(1000067, 0) } }
            ...
            if (_manage) {
                ...
                u.html("");
                u.append(v);
                a.homePoup(x) } else { q.push("gameId=", p.gameId);
                q.push("&openId=", p.openId);
                if (g_config.isDoubleGame) { q.push("&openIdB=", a.otherOpenId) }
                if (f.firstTouchAward) { $(".ajaxLoadBg").show();
                    $(".ajaxLoadBar").addClass("ajaxLoad");
                    $.ajax({ type: "post", url: p.awardUrl, data: q.join(""), error: function() { a.poupAjaxComplete(x) }, success: function(D) { a.poupAjaxComplete(x);
                            a.tlog("poupAward", D);
                            var B = $.parseJSON(D);
                            $("#Award_Round_Dot").hide();
                            a.logDog(1000011);
                            if (B.success) { a.showAwardDetail(B.list) } else { $("#awardInfo").html('<div style="line-height: 2.6rem; padding-left: 0.05rem;">暂无中奖纪录</div>') }
                            if (B.userInfo && (B.userInfo.ausername || B.userInfo.aphone || B.userInfo.aadress)) { $("#awardContactInfo").show();
                                if (B.userInfo.ausername) { $("#awardContactInfo .contactName").show();
                                    $("#awardContactInfo .contactName span").text(B.userInfo.ausername) }
                                if (B.userInfo.aphone) { $("#awardContactInfo .contactPhone").show();
                                ...
                                if (B.userInfo.aadress) {
                                    ...
                                    if (C.isOverflow) { $("#awardContactInfo .contactAddress span").text(C.returnValue + "...") } else { $("#awardContactInfo .contactAddress span").text(B.userInfo.aadress) }
        ...
        a.showAwardDetail = function(H) {
            ...
            for (var E = 0; E < H.length; E++) {
                ...
                if (s.genewxcard) {
                    var I = true;
                    var L = A.depositTime;
                    if (L && L < Date.parse(new Date())) { I = false }
                    J = (I ? "放入" : "打开") + "微信卡券";
                ...
                var w = A.level;
                var u = A.awardName;
                if (A.anwei) { w = A.awardLevel;
                    u = A.award;
                    v = (A.cusCode && A.cusCode.length > 0) ? A.cusCode : A.code1 }
                if (A.isTrash) { A.codeStatus = 4 }
                A.awardCode = v;
        ...
        a.openAwardDetail = function(t) {
            ...
            if (g_config.afterLinkModify && (a.shouldRegInfo(2, arguments, this) || a.shouldRegInfo(3, arguments, this) || a.shouldRegInfo(4, arguments, this))) {
                ...
                if (typeof q.continfo !== "undefined") {
                    if ((t && ((gameType == 0 || gameType == 4 || gameType == 5) && g_config.wxAward.oplink))) {
                        ...
                        if (w || u || y) { a.aUserInfo.show(arguments, this);
                            return } } } else {
                    if (!g_config.awardUsername && !g_config.awardPhone && !g_config.awardAddress && g_config.award && ((gameType == 0 || gameType == 4) && g_config.wxAward.oplink)) { a.aUserInfo.show(arguments, this);
                        return } } }
            var E = $("#awardDetailBox");
            var r = $("#ticketDetailBox");
            $("#codeStatusInfo").html("商家已将该兑奖码设置为作废<br>详情请联系商家");
            if (t.codeStatus == 4) { $("#codeStatusBtn").html("已作废");
                $("#codeStatusBox").show();
                $("#codeDetailInfoBox").hide() } else {
                if (t.codeStatus == 1) { $("#codeStatusBtn").html("已核销");
                    $("#codeStatusBox").show();
                    $("#codeDetailInfoBox").hide();
                    $("#codeStatusInfo").hide();
                    if (q.genewxcard) { $("#awardCollectionBtn").hide() } } else {
                    if (t.codeStatus == 5) { $("#codeStatusBtn").html("已失效");
                        $("#codeStatusInfo").html("红包已失效，详细请联系商家");
                        $("#codeStatusBox").show();
                        $("#codeDetailInfoBox").hide() } else {
                        if (t.codeStatus == 3) { $("#codeStatusBtn").html("已过期");
                            $("#codeStatusBox").show();
                            $("#codeDetailInfoBox").hide();
                            $("#codeStatusInfo").hide() } else {
                            if (t.theGiftDate >= 0) { $("#codeStatusBox").hide();
                                $("#codeDetailInfoBox").show();
                                $("#codeStatusInfo").show() } else {
                                var z = Math.ceil((t.theGiftDate * -1) / (1000 * 60 * 60 * 24));
                                var x = "";
                                if (q.awardtype == 0 || q.awardtype == 1) { x = "优惠券" }
                                if (q.awardtype == 2) { x = "兑奖券" }
                                if (q.awardtype == 5) { x = "口令码" }
            ...
            if (p.afterWxCard) { $(".awardCusText").show();
                $(".awardCusBtn").show();
                if (q.awardtype == 5 || q.awardtype == 1 || (q.awardtype == 2 && q.cashtype != 2)) { r.find(".addressLine").hide();
                    E.find(".awardCusText").show();
                    if (q.awardtype == 1) { E.find(".awardCusBtn .text").text("商城地址") } else { E.find(".awardCusBtn .text").text("兑奖地址") }
                    if (q.awardtype == 2 && q.cashtype == 1 && q.onlinect == 1) { $("#QRImg_1").attr("src", q.onlinewxnum);
                        $("#awardCusBtnInfo").attr("href", "###");
                        E.find("#awardCusBtnInfo").off("click").on("click", function() { $("#newAttentionMask").show() }) } else {
                        if (q.awardtype == 5 || (q.awardtype == 2 && q.cashtype == 3)) { $("#awardDetailBox").find(".codeOptInfo").hide();
                            ...
                            E.find("#attentionCashBtn").off("click").on("click", function() { $("#newAttentionMask").show() }) } else {
                            if (q.awardtype == 2 && q.cashtype == 4) { $("#webCashBtn").show();
                                $("#awardCusBtnInfo").hide();
                                $("#attentionCashBtn").hide();
                                $("#awardDetailBox").find(".wxCashOptInfo").hide();
                                $("#awardDetailBox").find(".codeOptInfo").show();
                                if (!a.isUrl(q.cashsite)) { q.cashsite = a.fixUrl(q.cashsite) }
                                $("#webCashBtn").attr("href", q.cashsite) } else { E.find("#awardCusBtnInfo").attr("href", q.cashsite) } } }
                    r.find(".itemList .address").html(q.cashsite);
                    if (!$.trim(q.servicepho) && !$.trim(q.cashinfo) && (!q.tlmt || q.tlmt == "[8]" || q.tlmt == "[1,2,3,4,5,6,7]")) { E.find(".awardCusText").hide() }
                    r.find(".itemList .timelimit").parents(".itemList").toggle(q.awardtype != 5);
                    if (q.awardtype == 5) { $(".wxCashOptInfo").text("复制口令发送到公众号");
                        $("#attentionCashBtn .text").text("马上领取") } else { $(".wxCashOptInfo").text("发送兑奖码到公众号进行兑奖");
                        $("#attentionCashBtn .text").text("关注兑奖") } } else { r.find(".itemList .address").text(q.cashsite.replace("http://", ""));
                    if (q.oct) { E.find(".awardCusBtn .text").text(q.btn);
                        E.find("#awardCusBtnInfo").attr("href", q.btl) } else { $(".awardCusBtn").hide() } }
                ...
                r.find(".itemList .notice").html('<pre style="word-wrap: break-word;">' + a.encodeHtml(q.cashinfo) + "</pre>") } else {
                if (q.optx) { E.find(".awardCusText .text").text(q.txn);
                    E.find("#awardCusTextInfo").text(q.txc) } else { $(".awardCusText").hide() }
                if (q.oct) { E.find(".awardCusBtn .text").text(q.btn);
                    E.find("#awardCusBtnInfo").attr("href", q.btl) } else { $(".awardCusBtn").hide() } }
            if (typeof q.cashtype === "undefined" || (q.awardtype != 1 && q.cashtype == 2)) { a.logDog(1000085);
                if (q.attention == 3) { $("#awardCusBtnInfo").attr("href", "###");
                    $("#QRImg_1").attr("src", q.attentionimg);
                    $("#awardCusBtnInfo").off("touchend").on("touchend", function() { a.logDog(1000086);
                        $("#newAttentionMask").show() }) } else {
                    if (q.attention == 2) { $("#awardCusBtnInfo").attr("href", q.btl) } } }
            a.log(q);
            if (typeof q.opqrc === "undefined" || q.opqrc) { $("#awardDetailBox .codeImg ").show() } else { $("#awardDetailBox .codeImg,#awardDetailBox .codeDetailImgBox").hide() }
            a.wxConfig.setWxShareUrlArg(["fromFav", t.awardCode]);
            a.logDog(1000056);
            a.logDog(1000045, 1);
            $("#awardDeailBg").show();
            $("#awardDetailBox").show();
            setTimeout(function() { $("#awardDeailBg").hide() }, 800) };
        a.shouldRegInfo = function(r, t, u) {
            var q = false;
            if (g_config.afterLinkModify) {
                if (g_config.linkInfoType != 1 && g_config.linkInfoType == r) { q = true }
                if (a._isZhuliPlayer && (g_config.linkInfoType == 2 || g_config.linkInfoType == 3)) { q = false } } else {
                if ((gameType == 3 || gameType == 1) && g_config.openAwardLinkNoDraw) { q = true } }
            if (q && !s()) { a.aUserInfo.show(t, u);
                return true }
            return false;
        ...
        function h(q) { $("#regAwardBox").show();
            if (_manage) { a.homePoup(q);
                var u = [];
                var t = ["lvox、hubert", '<span style="color:#fff000">magazine</span>、tina', "monica、hth"];
                var s = $(".regAwardList");
                s.find(".playerName").html("");
                s.each(function(v) { $(this).find(".playerName").html(t[v]) }) } else { a.logDog(1000034);
                var r = p.getRegAwardUrl;
                if (g_config.isDoubleGame) {
                    var r = r + "&openIdB=" + a.otherOpenId }
                if (f.firstTouchWinList) { $(".ajaxLoadBg").show();
                    $(".ajaxLoadBar").addClass("ajaxLoad");
                    $.ajax({ type: "post", url: r, error: function() { a.poupAjaxComplete(q) }, success: function(v) { a.tlog("poupRegAward", v);
                            var w = $.parseJSON(v);
                            ...
                            setTimeout(function() {
                                ...
                                $(w).each(function() {
                                    ...
                                    var C = F - 1;
                                    if (F == 900) { C = x.length - 1 }
                                    var E = "";
                                    if (D) { E = 'style="color:#fff000;"' }
                                    var B = "<span " + E + ">" + $(this)[0].name + "</span>";
                                    A[C].name.push(B) });
                                for (var y = 0; y < A.length; y++) { x.eq(y).find(".playerName").append(A[y].name.join("、")) }
                                $("#regAwardMain").show() }, 500);
                            f.firstTouchWinList = false } }) } } }

        function j(q) { $("#informBox").show();
            $(".unPublish").hide();
            $("#inform-reason-page").show();
            $("#inform-desc-page").hide();
            $("#next-inform-btn").hide();
            $("#inform-reason-box .reasonItem").removeClass("checked");
            $("#informResultBox .confirm-btn").off("touchend").on("touchend", function() { event.preventDefault();
                if (!_manage) { $(".gameBox,.home,.body").removeClass("overflow-y-hidden");
                    g_config.showSkillSup && $(".bottomSkill").show() }
                $("#informBox").hide();
                ...
                $("#poupInfoBox").hide();
                hg.fire("hidePoup", q) });
            $("#cancel-inform-btn").off("touchend").on("touchend", function() { event.preventDefault();
                var r = $("#poupInfoBox");
                r.removeClass("poupFadIn").addClass("poupFadOut");
                if (!_manage) { $(".gameBox,.home,.body").removeClass("overflow-y-hidden");
                    g_config.showSkillSup && $(".bottomSkill").show() }
        ...
        $("#next-inform-btn").on("touchend", function() { $("#inform-reason-page").hide();
        ...
        $("#submit-inform-btn").on("touchend", function() {
            if (!p.hasReport) {
                ...
                r = $.trim(r);
                if (r == null || r == "") { a.showMsgToast("描述不能为空", 2000);
                    return } else {
                    if (r.length > 50) { a.showMsgToast("填写超出限制", 2000);
                        return } else { $.ajax({ type: "post", url: p.informUrl, error: function(s) { a.tlog("submit-inform-btn-err", s) }, success: function(s) { a.tlog("submit-inform-btn", s);
                                p.hasReport = true;
                                $("#informResultBox h3").text("投诉成功");
                                $("#informResultBox").show() } }) } } } else { $("#informResultBox h3").text("你已经投诉过了！");
                $("#informResultBox").show() }
            var q = $("#poupInfoBox");
            q.removeClass("poupFadIn").addClass("poupFadOut");
            setTimeout(function() { $("#spxdPage").removeClass("imp-hide") }, 400) });
        $("#awardContactInfo .updateBtn").on("touchend", function() { a.aUserInfo.show() }) })();
    (function() {
        var f = false;
        a.sortRuleBox = { init: d, addBtn: c, };
        ...
        function c() { a.addModuleLayer("#ruleBox .poupLine", { name: "下移", onshow: function(g) {
                    var h = g.target;
                    if (h.next().length == 0) { $(this).hide() } else { $(this).show() } }, onclick: function(h) {
                    var i = h.data.target;
                    var g = i.next();
                    if (g.length != 0) { e(i, g) } } });
            a.addModuleLayer("#ruleBox .poupLine", { name: "上移", onshow: function(g) {
                    var h = g.target;
                    if (h.prev().length == 0) { $(this).hide() } else { $(this).show() } }, onclick: function(h) {
                    var i = h.data.target;
                    var g = i.prev();
                    if (g.length != 0) { e(g, i) } } }) }
        ...
        function e(i, h) {
            ...
            var g = i.height();
            var j = h.height();
            i.animate({ top: j + 0.7 * g_rem }, 300);
            h.animate({ top: -g - 0.7 * g_rem }, 300, function() { f = false;
                i.insertAfter(h);
                i.css("top", 0);
                h.css("top", 0) }) } })();
    (function() {
        var c = {};
        a.lastDisplayStatus = "";
        c.show = function(d, f) { a.lastDisplayStatus = $(".gameBgBox").css("display");
            $(".gameBgBox").hide();
            $("#awardUserInfoBg").data({ _aUserInfoArg: d, _aUserInfoThis: f }).show();
            var e = g_config.ajaxUrl.replace("/ajax/", "");
            if (window.top.$("#contactInputIframe").length == 0) { $("#awardUserInfoBg").append('<iframe id="contactInputIframe" frameborder="0" scrolling="no" src="' + e + "/contactInfo.jsp?afterLinkModify=" + g_config.afterLinkModify + "&awardLinkMsg=" + a.encodeUrl(a._awardLinkMsg) + '" style="height: 100%; width: 100%;"></iframe>') }
            ...
            if (!d) { $("#awardUserInfoBg").removeData("_aUserInfoArg") } };
        c.hide = function() { window.top.$(".gameBgBox").css("display", parent.HdGame.lastDisplayStatus);
            ...
            if (d(h)) {
                var g = h[2];
                if (g && g.info) { g.info = $.parseJSON(g.info);
            ...
            h.callee.apply(f, h);
            setTimeout(function() { c.afterUserInfoHide = false }, 0);
            setTimeout(function() { window.top.$("#contactInputIframe").remove() }, 510);
    ...
    a.showLotsWait = function(c, e) {
        ...
        d.text(c);
        $("#lots-wait-Box").show();
        var f = setInterval(function() { c--;
            d.text(c);
            if (c <= 0) { clearInterval(f);
                $("#lots-wait-Box").hide();
                e() } }, 1000) };
    (function() {
        var c = Math.max($(window).height() / $(window).width(), $(window).width() / $(window).height()) > 22 / 16;
        var g = { startStatus: false, exposeFlag: false, disable: false, result_disable: false, gift_disable: false, };
        var f = { drawType: false, home: function() {}, again: function() {}, giftInit: function(h) { h && h() } };
        var e = (function() {
            var m = 1.5;
            var j = 0;
            var i, o, n;
            ...
            return function(q) {
                ...
                if ((s - j) > 100) { $("input").blur();
                    ...
                    if (r > m) { g.startStatus = false;
                        f.giftInit(function(t) { hg.sound.play("yiy");
                            setTimeout(function() { g.showGift(t) }, 1500) }) }
                    l = i;
                    k = o;
                    h = n } } })();
        var d = {};
        ...
        g.init = function(h) {
            ...
            $("#lots-wait-img").css({ width: i / (6.15 * g_rem) * m, height: k / (12.4 * g_rem) * j, "background-image": 'url("' + n + '")' });
            if (a.currentScore >= g_config.scoreSet && g_config.scoreSet != "") { a.isplaySucess = true } else { a.isplaySucess = false }
            $(".resule-status-home , .resule-gift-home, .resule-gift-home2").on("touchend", function() { event.preventDefault();
                event.stopPropagation();
                if (_manage) {
                    return }
                a.logDog(1000015);
                a.fadIn(l.resuleBox, function() { l.giftBox.hide();
                    l.statusBox.hide();
                    $(".gameBox,.home,.body").removeClass("overflow-y-hidden");
                    if (g_config.showSkillSup) { a.logDog(1000028, 0);
                        $(".bottomSkill").show();
                        if (g_config.aid == 7171826) { a.logDog(1000062, 2) } else {
                            if (g_config.aid == 8566249) { a.logDog(1000067, 2) } } } });
                f.home() });
            $("#resule-status-lotsHand .waitDrawBtn").off("touchend").on("touchend", function() { g.startStatus = false;
                f.giftInit(function(o) { l.showGift(o) }) });
            $(".resule-gift-draw").off("touchend").on("touchend", function() {
                ...
                if (l.resuleArg.isLimitDrawTotal && l.resuleArg.totalCount === 0) { a.statusMsg(6);
                    return } else {
                    if (l.resuleArg.count === 1) { a.statusMsg(4);
                        if (!g_config.showHelpGuide) { f.home() }
                        return } }
                $("#resule-status-scrollWrap,#resule-gift-box").hide();
                $("#resule-status-lotsBox,#resule-status-box").show();
                setTimeout(function() { g.startStatus = true;
                    l.resuleArg.count > 0 && l.resuleArg.count-- }, 1000);
                ...
                a.lotTimer = setTimeout(function() {
                    if (drawStatusLuckDraw) { $("#resule-status-lotsHand .shakeHand,#resule-status-lotsHand .shakeTxt").hide();
                        $("#resule-status-lotsHand .waitDrawBtn").show() } }, 5000);
                g.exposeFlag = false });
            if (f.drawType === 0 || f.drawType === 2) { $(".resule-gift-home").css({ width: "10rem", height: "2rem", "line-height": "2rem" });
                $(".resule-gift-home2").css({ width: "4.75rem", height: "2rem", "line-height": "2rem" });
                this.giftBox.find(".resule-gift-seeRank").remove();
                this.show = this.showGift } else {
                if (f.drawType === 1) { $(".resule-status-seeRank,.resule-gift-seeRank").on("touchstart", function() { event.preventDefault();
                        event.stopPropagation();
                        if (_manage) {
                            return }
                        window.showRank() });
                    $(".resule-status-again").on("touchstart", function() { event.preventDefault();
                        event.stopPropagation();
                        if (_manage) {
                            return }
                        a.logDog(1000016);
                        if (gameType == 3 && g_config.isCheckPlayTimes && PlayInfo.getTotalRemainTimes() <= 0) { a.statusMsg(8);
                            return }
                        if (gameType == 3 && g_config.isCheckPlayTimes && PlayInfo.getTodayRemainTimes() <= 0) { a.statusMsg(7);
                            return }
                        $(".gameBox,.home,.body").removeClass("overflow-y-hidden");
                        if (!g_config.isDoubleGame) { a.fadIn(l.resuleBox, function() { l.giftBox.hide();
                                l.statusBox.hide() }) }
                        f.again() });
                    /*hg.sound.load(_resRoot + "/image/yaoyiyao.wav", "yiy");*/
                    hg.sound.load("music/yaoyiyao,wav", "yiy");
                    window.addEventListener("devicemotion", e, false);
                    if (m_debug || g_config.test) { $("#resule-status-lotsHand").on("touchstart", function() { g.startStatus = false;
                            f.giftInit(function(o) { l.showGift(o) }) }) }
                    this.giftEvent = function() {
                        ...
                        if (l.resuleArg.isLimitDrawTotal && l.resuleArg.totalCount === 0) { a.statusMsg(6);
                            return } else {
                            if (l.resuleArg.count === 0) { a.statusMsg(4);
                                return } }
                    ...
                    this.regEvent = function() {
                        ...
                        $(".ajaxLoadBar").addClass("ajaxLoad");
                        var o = f.regUrl;
                        if (g_config.isDoubleGame) { o += "&openIdB=" + a.otherOpenId }
                        $.ajax({ type: "post", url: o, error: function() { a.otherAjaxComplete() }, success: function(q) {
                                var p = $.parseJSON(q);
                                a.otherAjaxComplete();
                                if (p.isOutofTestNum) { a.statusMsg("活动尚未发布", "最多测试人数为100人！");
                                    return }
                                if (!p.isOutofRegNum) { a.isReg = true;
                                    var p = $.parseJSON(q);
                                    if (p.success) { $(".resule-foot-one .resule-status-again").html("继续刷记录");
                                        $(".resule-foot-one .resule-status-reg").hide();
                                        $(".resule-foot-one .resule-status-again").show();
                                        $("#resule-status-count").hide();
                                        $("#resule-sucReg").show() } } else { a.statusMsg("报名人数已经满了") } } }) };
        ...
        g.showResult = function(k) {
            if (this.disable || this.result_disable) {
                return }
            var j = { isSuc: false, gameScore: 0, minScore: 0, bestScore: 10, rank: 10, count: 3, beat: 99, notreal: false, gameType: 0 };
            var i = $.extend(j, k);
            ...
            if (!i.notreal && g_config.sortType == (i.gameScore >= g_config.drawLimitDef)) {
                if (typeof a.oss_aof === "undefined") { a.oss_aof = 0 }
                a.oss_aof++;
                if (a.oss_aof >= 2) { a.logDog(1000033, g_config.failSrcId);
                    a.oss_aof = 0 } }
            ...
            if (g_config.aid == 7171826 && g_config.showSkillSup) { a.logDog(1000062, 0) } else {
                if (g_config.aid == 8566249 && g_config.showSkillSup) { a.logDog(1000067, 0) } }
            if (i.gameType == 1 || i.gameType == 3) { g_config.showMenu && a.logDog(1000036) }
            if (!_manage) { $(".gameBox,.home,.body").addClass("overflow-y-hidden");
                $(".bottomSkill").is(":visible") && $(".bottomSkill").hide() }
            if (i.gameType == 3) { $("#resule-foot-box").css("padding-bottom", "1rem");
                if (i.gameScore == "fail") { $("#resule-status-bird").show();
                    ...
                    if (i.rank == 0) {} else { $("#resule-status-body .result-scoreUnit").show() }
                    l.eq(0).text("无");
                    if (l.eq(2).text().length == 0) { l.eq(2).text(i.bestScore);
                        l.eq(3).text(i.rank > 0 ? i.rank : "无") } else {
                        if (l.eq(0).text() == "无") { $("#resule-status-body .youraward .result-scoreUnit").hide() } else { $("#resule-status-body .result-scoreUnit").show() } }
                    a.isplaySucess = false } else { $("#resule-status-bird").hide();
                ...
                if (gameType == 3 && g_config.isCheckPlayTimes) { $("#resule-status-playinfo").show() } else { $("#rank_showRule").show() } } else {
                if (i.gameScore === "fail") { $(".resuleArg-fail").text(i.minScore);
                    $("#resule-status-body").hide();
                    $("#resule-status-other").show() } else { $("#resule-status-body").show();
                    $("#resule-status-other").hide() }
                if (i.isSuc) { a.isplaySucess = true;
                    $("#resule-status-bird").hide();
                    $("#resule-status-ribbon").removeClass("resule-status-faiRibbon").addClass("resule-status-ribbon");
                    $(".resule-status-userImg").css("border-color", "#70D572");
                    $(".resule-status-minscore").hide();
                    $("#resule-status-count").show();
                    if (i.gameType == 0) { $(".resule-foot-one .resule-status-gift").show();
                        $(".resule-foot-one .resule-status-again").hide();
                        $(".resule-foot-two .resule-status-again").show();
                        $(".resule-foot-two .resule-status-home").hide();
                        a.logDog(1000027) } else {
                        if (i.gameType == 4) { $(".resule-foot-one .resule-status-send").show();
                            $(".resule-foot-two .resule-status-again").hide();
                            $(".resule-foot-two .resule-status-home").show();
                            $("#resule-status-count").hide() } else {
                            if (i.gameType == 1) { $("#resule-status-count").hide();
                                $("#resule-foot-box").css("margin-top", "0.6rem");
                                $(".resule-foot-two .resule-status-again").hide();
                                $(".resule-foot-two .resule-status-home").show();
                                a.logDog(1000035);
                                if (!h) { $(".resule-foot-one .resule-status-reg").show() } else { $(".resule-foot-one .resule-status-again").html("继续刷记录");
                                    $(".resule-foot-one .resule-status-reg").hide();
                                    $(".resule-foot-one .resule-status-again").show();
                                    $("#resule-sucReg").show() } } } }
                    g.exposeFlag = true;
                    ...
                    $("#resule-status-body .beat-Percent span").text(i.beat);
                    $("#resule-status-body .beat-Percent").show() } else { $("#resule-status-bird").show();
                    $("#resule-status-ribbon").removeClass("resule-status-ribbon").addClass("resule-status-faiRibbon");
                    $(".resule-status-userImg").css("border-color", "#B5B5B5");
        ...
        g.showGift = function(l) {
            if (this.disable || this.gift_disable) {
                return }
            var j = { isSuc: false, giftName: "", giftStyle: "", giftCode: 0, giftImage: "", awardImageW: "6rem", awardImageH: "6rem", awardTypeNum: 0, awardIndex: 1, genewxcard: false };
            var s = $.extend({}, j, l);
            if (this.statusBox) { a.fadIn(this.statusBox) }
            $("#resule-gift-sucImg").data("openCode", s.giftCode);
            a.fadOut(this.resuleBox);
            ...
            if ((gameType == 0 || gameType == 4 || gameType == 5) && !_manage) {
                ...
                if (t > i) { $(".cannotGetThePriceBox").height(i);
                    ...
                    if (r > k) {
                        if ((k * q) > n) { $("#faiImgBox img").height(n / q).width(n) } else { $("#faiImgBox img").height(k).width(k * q) } } } }
            $(".TellToOther").toggle(s.isSuc);
            $(".menuAgain,.menuBack,.backListen,.repeatDraw,.TellToOther").css({ bottom: $("#resule-gift-box").find(".attentionBox").height(), "line-height": "1.9rem" });
            ...
            if (g_config.aid == 7171826 && g_config.showSkillSup) { a.logDog(1000062, 0) } else {
                if (g_config.aid == 8566249 && g_config.showSkillSup) { a.logDog(1000067, 0) } }
            g_config.showMenu && a.logDog(1000036);
            if (s.isSuc) { $("#resule-gift-box").css("height", "auto");
                ...
                $("#resule-gift-sucImg").css({ width: s.awardImageW, height: s.awardImageH, margin: "1.6rem auto" });
                $("#lightPoint").css("top", parseFloat(s.awardImageH) / 2 + 1.8 + "rem");
                $("#click-view").css("top", parseFloat(s.awardImageH) / 2 + 2.5 + "rem");
                if (s.awardIndex == 900) {
                    if (g_config.comfort.comAwardtype == 0 || g_config.comfort.comAwardtype == 1) { $(".awardCodeTime em").text("使用期限");
                        $("#codeName").text("优惠码");
                        if (g_config.afterWxCard) { $(".awardCusText .text").text("优惠券详情") } } else { $(".awardCodeTime em").text("兑奖期限");
                        $("#codeName").text("兑奖码");
                        $("#awardCollectionBtn").text("收藏兑奖券");
                        if (g_config.afterWxCard) { $(".awardCusText .text").text("兑奖券详情") } }
                    $("#resule-gift-sucImg").attr("src", g_config.comfort.comimg) } else {
                    if (!g_config.award) { g_config.award = g_config.awardList[s.awardIndex - 1];
                        g_config.wxAward = g_config.awardList[s.awardIndex - 1] }
                    $("#resule-gift-sucImg").attr("src", g_config.awardList[s.awardIndex - 1].aimg);
                    if (s.awardTypeNum == 0 || s.awardTypeNum == 1) { $(".awardCodeTime em").text("使用期限");
                        $("#codeName").text("优惠码");
                        if (g_config.afterWxCard) { $(".awardCusText .text").text("优惠券详情") } } else { $(".awardCodeTime em").text("兑奖期限");
                        $("#codeName").text("兑奖码");
                        $("#awardCollectionBtn").text("收藏兑奖券");
                        if (g_config.afterWxCard) { $(".awardCusText .text").text("兑奖券详情") } }
                    if (g_config.award.genewxcard) { $(".awardCodeTime em").text("兑奖期限");
                        $("#codeName").text("兑奖码");
                        var m = true;
                        var o = awardInfo.depositTime;
                        if (o && o < Date.parse(new Date())) { m = false }
                        var h = (m ? "放入" : "打开") + "微信卡券";
                        $("#awardCollectionBtn").text(h).attr("isWxCard", true) } }
                a.logDog(1000003, 0);
                a.logObjDog(1000092, 3, g_config.gameId);
                if (!_manage) { $(".gameBox,.home,.body").addClass("overflow-y-hidden");
                    $(".bottomSkill").is(":visible") && $(".bottomSkill").hide() } } else { $("#resule-gift-box").css("height", $(window).height() - $("#resule-gift-box .attentionBox").height());
                $("#resule-gift-scrollWrap,#resule-status-lotsBox").hide();
                $("#faiImgBox").show() }
            if (g_config.haveAward) { $("#myAwardBtn").show() } };
        a.resulePoup = g })();
    a.otherAjaxComplete = function() {
        var c = top.$(".ajaxLoadBar");
        c.addClass("ajaxComplete");
        setTimeout(function() { c.removeClass("ajaxLoad");
            c.removeClass("ajaxComplete");
            top.$(".ajaxLoadBg").hide() }, 500) };
    (function() {
        var i = (function() {
            var j = 1;
            return function(k) {
        ...
        function e(k) {
            ...
            var j = "";
            $.each(m, function(n, o) {
                if ($("#" + k + "moveFrame" + o).length == 0) { j += '<div id="' + k + "moveFrame" + o + '" class="moveFrame" style="border-' + o.toLowerCase() + '-width:1px"></div>' } });
            if (j) { $("body").append(j);
                f(l) } }
        ...
        function f(j, p) {
            ...
            var q = j.outerWidth();
            var n = j.outerHeight();
            $("#" + k + "moveFrameLeft").css({ left: m, top: s, width: 0, height: n, borderLeftWidth: 1 });
            $("#" + k + "moveFrameRight").css({ left: m + q - 1, top: s, width: 0, height: n, borderRightWidth: 1 });
            $("#" + k + "moveFrameTop").css({ left: m, top: s, width: q, height: 0, borderTopWidth: 1 });
            $("#" + k + "moveFrameBottom").css({ left: m, top: s + n - 1, width: q, height: 0, borderBottomWidth: 1 });
            if (p) {
                var u = j.closest(".ui-wrapper");
                if (u.length == 0) { u = j }
                var r = u.find(".recoverBtn");
                var o = false;
                if (q < 2 * r.width()) { o = true }
                if (n < 2 * r.height()) { o = true }
                if (o && !j.data("_minRecoverBtn")) { j.data("_minRecoverBtn", true);
                    u.find(".ui-resizable-imgTopRightTip").css({ backgroundPositionX: 0, backgroundPositionY: 11, right: -11, top: -11 });
                    u.find(".ui-resizable-imgTopLeftTip").css({ backgroundPositionX: 11, backgroundPositionY: 11, left: -11, top: -11 });
                    u.find(".ui-resizable-imgBottomLeftTip").css({ backgroundPositionX: 11, backgroundPositionY: 0, left: -11, bottom: -11 });
                    u.find(".ui-resizable-imgBottomRightTip").css({ backgroundPositionX: 0, backgroundPositionY: 0, right: -11, bottom: -11 });
                    r.css("right", -24) } else {
                    if (j.data("_minRecoverBtn")) { j.data("_minRecoverBtn", false);
                        u.find(".ui-resizable-imgTopRightTip").css({ backgroundPositionX: 11, backgroundPositionY: 0, right: 0, top: 0 });
                        u.find(".ui-resizable-imgTopLeftTip").css({ backgroundPositionX: 0, backgroundPositionY: 0, left: 0, top: 0 });
                        u.find(".ui-resizable-imgBottomLeftTip").css({ backgroundPositionX: 0, backgroundPositionY: 11, left: 0, bottom: 0 });
                        u.find(".ui-resizable-imgBottomRightTip").css({ backgroundPositionX: 11, backgroundPositionY: 11, right: 0, bottom: 0 });
                        r.css("right", 3) } } } }
        a.removeMoveFrame = function(j) { $("#" + j + "moveFrameLeft,#" + j + "moveFrameRight,#" + j + "moveFrameTop,#" + j + "moveFrameBottom").remove();
            $$("#imgSizeShow").hide() };
        a.removeAllEditLayer = function() { $(".moveFrame").remove();
            $(".moduleLayer").remove() };
        a.initModuleLayer = d(function(m, l, o, k, n, p, j) { i(m);
            ...
            m.off("mouseover").off("mouseleave").on("mouseover", m.data("mouseover")).on("mouseleave", function(r) {
                ...
                $(this).data("initModuleTimer", setTimeout(function() {
                    if (!a.ResizeDoc) { a.removeMoveFrame(q);
                        $("#" + q + "moduleLayer").remove() } }, 100)) });
            if (j || l != -2) { a.addModuleLayer(m, { name: typeof j === "string" ? j : "编辑", onclick: function(r) { parent.Edit.showEditByGame.call(r.data.target, l, o, k);
                        ...
                        return false } }) }
            if (n) { a.addModuleLayer(m, { name: typeof n === "string" ? n : "样式", onclick: function(q) { parent.Edit.Css.showCssByGame.call(q.data.target, o, p);
        ...
        function h(k) {
            ...
            j.css("margin", 0);
            g(p, j);
            j.mouseover(function() { clearTimeout(p.data("initModuleTimer")) }).mouseleave(function(s) { p.data("initModuleTimer", setTimeout(function() { a.removeMoveFrame(k);
                    $("#" + o).remove();
                    var t = p.siblings(".ui-resizable-handle,.recoverBtn");
                    t = (t.length > 0) ? t : (p.find(".ui-resizable-handle,.recoverBtn"));
                    t.siblings(".ui-resizable-handle,.recoverBtn").hide() }, 100)) });
            return j }
        ...
        function g(m, n) {
            ...
            if (k < 0) { k = 0 }
            if (m.offset().left < $(window).width() && m.offset().left + j > $(window).width()) { k = $(window).width() - n.outerWidth() }
            n.css("left", k);
            var l = (m.offset().top - n.height() - 1);
            n.css("top", l > 0 ? l : 0 + "px") }
        a.moduleSlaveImgResize = d(function(m, n, o) {
            var j = o || "body";
            if (m.parents(".home").length > 0 || m.parents(".zlEnterHome").length > 0) { j = "#limitRange" }
            m.parent("div.ui-wrapper").css({ padding: "0" });
            m.resizable("destroy");
            ...
            i(l);
            l.data("defData", n);
            m.resizable($.extend({ handles: "n,e,s,w,ne,nw,sw,se", autoHide: true, maxWidth: 318, maxHeight: 504, minWidth: 20, minHeight: 20, aspectRatio: true, containment: j, alsoResize: m, create: function(p, r) {
                    var t = $(this);
                    var q = 0;
                    if (Math.abs(t.width() - parseRemToPx(n.width)) > 1 || Math.abs(t.height() - parseRemToPx(n.height)) > 1) { l.attr("resize", 1) }
                    t.css({ position: "relative", "float": "none", "z-index": "100" });
                    if (t.width() < 40 || t.height() < 40) { t.css("padding", "2px") } else { t.css("padding", "0") }
                    t.append("<a class='recoverBtn' href='javascript:;'></a>");
                    t.find(".recoverBtn").on("click", function(u) { $(this).hide();
                        l.attr("resize", 0);
                        t.add(l).css(n);
                        a.logDog(1000078, 4);
                    ...
                    (function() {
                        ...
                        for (var v = 0; v < 4; v++) { t.children(".ui-resizable-" + w[v][0]).addClass("ui-resizable-img" + w[v][1] + "Tip").css({ "z-index": "9030" });
                            if (v < 2) {
                                for (var u = 2; u < 4; u++) { t.children(".ui-resizable-" + w[v][0] + w[u][0]).addClass("ui-resizable-img" + w[v][1] + w[u][1] + "Tip").css({ "z-index": "9030" }) } } } })();
                    var s = t.parent();
                    t.children(".ui-resizable-handle").mousedown(function(v) {
                        var u = s.attr("href");
                        if (typeof u != "undefined" && u.length > 0) { s.removeAttr("href");
                            s.attr("onclick", "return false;") } });
                    if (l.data("layerId")) { l.off("mouseover").off("mouseleave") }
                    t.mouseover(function() { clearTimeout(l.data("initModuleTimer"));
                        if (a.ResizeDoc) {
                            return }
                        t.children(".ui-resizable-handle").show();
                        if (l.attr("resize") == 1) { k.find(".recoverBtn").show() }
                        if (l.data("layerId")) { l.data("mouseover").call(l) }
                        l.removeData("resizable-isLeave") }).mouseleave(function() { l.data("initModuleTimer", setTimeout(function() {
                            if (!a.ResizeDoc) {
                                var u = l.data("layerId");
                                a.removeMoveFrame(u);
                                $("#" + u + "moduleLayer").remove();
                                t.children(".ui-resizable-handle").hide();
                                k.find(".recoverBtn").hide() } else { l.data("resizable-isLeave", true) } }, 100)) });
                    l.data("hd-resizable-create") && l.data("hd-resizable-create").fireWith(l[0], [p, r]) }, start: function(p, q) {
                    if (q.axis.length === 2) { $(this).resizable("option", "aspectRatio", true) } else { $(this).resizable("option", "aspectRatio", false) }
                    e(l.data("layerId"));
                    a.ResizeDoc = k;
                    $$("#imgSizeShow").show();
                    ...
                    $(".ui-resizable-bottomTip,.ui-resizable-cancelTip,.draggableBtn").css({ opacity: "0" });
                    l.data("hd-resizable-start") && l.data("hd-resizable-start").fireWith(l[0], [p, q]) }, resize: function(q, r) { l.data("hd-resizable-resize") && l.data("hd-resizable-resize").fireWith(l[0], [q, r]);
                    f(l, true);
                    var p = $$("#imgSizeShow");
                    p.css({ top: q.pageY + 15, left: q.pageX - 30 });
                    p.find("span").eq(0).text(Math.round(l.width()) * 2);
                    p.find("span").eq(1).text(Math.round(l.height()) * 2) }, stop: function(q, s) {
                    var r = 0;
                    delete a.ResizeDoc;
                    if (l.data("resizable-isLeave")) { $(this).mouseleave();
                        l.removeData("resizable-isLeave") } else { l.mouseover() }
                    $$("#imgSizeShow").hide();
                    $(".ui-resizable-bottomTip,.ui-resizable-cancelTip,.draggableBtn").css({ opacity: "1" });
                    k.find(".slaveImg").attr("resize", "1");
                    k.find(".recoverBtn").show();
                    if ($(this).width() < 40 || $(this).height() < 40) { $(this).css("padding", "2px") } else { $(this).css("padding", "0") }
                    var t = $(this).parent();
                    var p = t.attr("link");
                    if (typeof p != "undefined" && p.length > 0) { setTimeout(function() { t.attr("href", p);
                            t.removeAttr("onclick") }, 100) }
                    parent.operateFlagList[17] = true;
                    a.logDog(1000078, 0);
                    l.data("hd-resizable-stop") && l.data("hd-resizable-stop").fireWith(l[0], [q, s]) } }, l.data("hd-resizable-option")));
            m.off(".slaveImg") });
        a.moduleDraggale = d(function(o, n, p) {
            ...
            if (l.parents(".home").length > 0 || l.parents(".zlEnterHome").length > 0) { k = "#limitRange" }
            var j = parseInt(m.css("z-index"));
            l.draggable($.extend({ start: function(q, r) { o.mouseover();
                    m.children(".ui-resizable-handle").hide();
                    j = parseInt(m.css("z-index"));
                    m.css("z-index", "9999");
                    o.data("hd-draggable-start") && o.data("hd-draggable-start").fireWith(o[0], [q, r]) }, drag: function(q, r) { f(o);
                    $("#" + o.data("layerId") + "moduleLayer").remove();
                    o.parent().find(".recoverBtn").hide();
                    o.data("hd-draggable-drag") && o.data("hd-draggable-drag").fireWith(o[0], [q, r]) }, stop: function(q, r) { m.css("z-index", j);
                    o.mouseover();
                    parent.operateFlagList[17] = true;
                    a.logDog(1000078, 1);
                    o.data("hd-draggable-stop") && o.data("hd-draggable-stop").fireWith(o[0], [q, r]) }, opacity: 0.5, scroll: false, distance: 1, containment: k, addClasses: false }, o.data("hd-draggable-option"))) });
        ...
        function d(j) {
            return function() {
                ...
                if (a.getType(l) === "array") {
                    if (a.getType(l[0]) === "array") { $.each(l, function(o, p) {
                            if (a.getType(p) === "array") { k.call(n, p) } }) } else { k.apply(n, l) } } else {
                    (typeof l === "string" || l.nodeType) && (l = $(l));
                    if (l instanceof jQuery && l.length > 0) {
                        var m = Array.prototype.slice.call(arguments);
                        if (l.length > 1) { l.each(function() { m[0] = $(this);
                                j.apply(n, m) }) } else { m[0] = l;
                            j.apply(n, m) } }
                    return n } } }
        ...
        function c(n, j, m, k) {
            ...
            n.each(function() {
                var o = $(this).data(l);
                if (k === "option") {
                    if (o) { $(this).data(l, $.extend({}, o, j)) } else { $(this).data(l, j) } } else {
                    if (!o) { o = $.Callbacks("unique memory");
                        $(this).data(l, o) }
                    o.add(j) } }) }
        $.fn.extend({ setResizableOption: function(j) { c($(this), j, "resizable", "option");
                return $(this) }, setDraggableOption: function(j) { c($(this), j, "draggable", "option");
                return $(this) }, addResizableFn: function(k, j) { c($(this), j, "resizable", k);
                return $(this) }, addDraggableFn: function(k, j) { c($(this), j, "draggable", k);
                return $(this) }, addEditUploadFn: function(k, j) { c($(this), j, "editUpload", k);
                return $(this) }, awardSlide: function(k) {
                if (typeof k === "object") {
                    var m = { margin_bottom: "0", padding_right: "0", padding_right: "0", speed: 15 };
                    var k = $.extend(m, k);
                    this.each(function() { $(this).show();
                        ...
                        var r = 0;
                        var t = 8;
                        $(".awardInfoList .uerItem").css({ "padding-left": k.padding_left, "padding-right": k.padding_right });
                        var u = parseInt($(".awardInfoList .uerItem").css("padding-left"));
                        var q = parseInt($(".awardInfoList .uerItem").css("padding-right"));
                        $(".awardInfoList .uerItem").each(function() { r += $(this).outerWidth(true) });
                        if (_manage) { r = 722 }
                        s.css({ width: r * 2 + 10 });
                        ...
                        p.css({ width: "50%", left: o + 20, "animation-duration": t + "s", "-webkit-animation-duration": t + "s", "-ms-animation-duration": t + "s" });
                        setTimeout(function() { p.eq(1).addClass("bottomScrollAnima") }, t / 2 * 1000);
                        $(this).data("awardSlideOptions", t) }) } else {
                    if (k === "stop") { $(this).hide();
                        $(this).find("li").eq(1).remove();
                        $(this).removeData("awardSlideOptions") } else {
                        if (k === "wakeup") { $(this).show();
                            var l = $(this).data("awardSlideOptions");
                            var j = $(this).children("ul").find("li");
                            j.eq(1).removeClass("bottomScrollAnima");
                            setTimeout(function() { j.eq(1).addClass("bottomScrollAnima") }, l / 2 * 1000) } } }
                return $(this) }, });
        a.Img = { MODE_SCALE_FILL: 1, MODE_SCALE_WIDTH: 2, MODE_SCALE_HEIGHT: 3, MODE_SCALE_DEFLATE_WIDTH: 4, MODE_SCALE_DEFLATE_HEIGHT: 5, MODE_SCALE_DEFLATE_FILL: 6, MODE_SCALE_DEFLATE_MAX: 7 };
        ...
        a.Img.optimize = function(n, p, l) {
            ...
            var m = k.width;
            var j = k.height;
            if (a.Img.isNull(m) || m == 0 || a.Img.isNull(j) || j == 0) { m = n.width;
                j = n.height }
            var o = a.Img.calcSize(m, j, p.width, p.height, p.mode, l);
            n.width = o.width;
            n.height = o.height;
            if (p.display == 1) { n.style.display = "inline" } else {
                if (p.display == 2) { n.style.display = "none" } else { n.style.display = "block" } }
            return { width: n.width, height: n.height } };
        a.Img.calcSize = function(j, q, o, p, l, k) {
            if (isNaN(o)) { o = parseFloat(o) * g_rem }
            if (isNaN(p)) { p = parseFloat(p) * g_rem }
            var r = { width: j, height: q };
            if (l == a.Img.MODE_SCALE_FILL) {
                var n = j / o;
                var m = q / p;
                if (n > m) { r.width = o;
                    r.height = q / n } else { r.width = j / m;
                    r.height = p } } else {
                                    ...
                                    if (l == a.Img.MODE_SCALE_DEFLATE_MAX) {
                                        if (j > o && q > p) {
                                            ...
                                            if (n < m) { r.width = o;
                                                r.height = q / n } else { r.width = j / m;
                                                r.height = p } } } } } } } } }
            if (!k) { r.width = Math.floor(r.width);
    ...
    (function() {
        ...
        $(function() {
            ...
            l.append(j);
            l.append(k);
            e.on("touchstart", function(m) { m.preventDefault();
                m.stopPropagation();
                var n = m.originalEvent.targetTouches[0];
                c.x = n.pageX;
                c.y = n.pageY }).on("touchmove", function(m) { m.preventDefault();
                m.stopPropagation();
                ...
                c.x = p.pageX;
                c.y = p.pageY;
                if (o > -0.2 * j.width() && o + 0.5 * j.width() < $(window).width()) { j[0].style.left = o + "px" }
                if (n > 0 && n + 0.5 * j.height() < $(window).height()) { j[0].style.top = n + "px" } });
            j.find("#log_close").on("touchstart", function() { k.hide();
                j.hide();
                g.empty();
                h = 0 });
            j.find("#log_min").on("touchstart", function() { k.show();
                j.hide() });
            j.find("#log_share").on("touchstart", function() {
                var n = a.wxConfigArg.url;
                a.log("shareUrl", n);
                if (g_config.test) { n = escape(n).replace(/\+/g, "%2B").replace(/\"/g, "%22").replace(/\'/g, "%27").replace(/\//g, "%2F");
                    var m = "http://" + a.gameDomainIgnoreStyle + "/hdtest.jsp?cmd=setUrl&url=" + n;
                    window.open(m) } });
            k.on("touchstart", function() { k.hide();
                j.show() });
            g.on("touchstart", ".log_flag", function() {
                var m = $(this).parent();
                if (m.hasClass("log_line_ellipsis")) { m.removeClass("log_line_ellipsis") } else { m.addClass("log_line_ellipsis") } });
            j.hide();
            k.hide() });
        a.tlog = d(function(m, l) {
            if (/log/.test(g_config.testCMD)) { a.logStd(m, l) }
            a.log(m, l) });
        ...
        function d(l) {
            return function(n, m) {
                ...
                if (arguments.length <= 1) { m = n;
                    n = "###" }
                if (a.getType(m) === "object" || a.getType(m) === "array") { m = $.toJSON(m) } else { m = String(m) }
                l(n, m) } } })();
    (function(h) {
        function p(v, A) {
        ...
        function f(G, B) { G[B >> 5] |= 128 << ((B) % 32);
            G[(((B + 64) >>> 9) << 4) + 14] = B;
            var y, A, z, w, v, F = 1732584193,
                E = -271733879,
                D = -1732584194,
                C = 271733878;
            for (y = 0; y < G.length; y += 16) { A = F;
                z = E;
                w = D;
                v = C;
        ...
        function q(w) {
            var x, v = "";
            for (x = 0; x < w.length * 32; x += 8) { v += String.fromCharCode((w[x >> 5] >>> (x % 32)) & 255) }
            return v }

        function k(w) {
            var x, v = [];
            v[(w.length >> 2) - 1] = undefined;
            for (x = 0; x < v.length; x += 1) { v[x] = 0 }
            for (x = 0; x < w.length * 8; x += 8) { v[x >> 5] |= (w.charCodeAt(x / 8) & 255) << (x % 32) }
            return v }
        ...
        function g(x, A) {
            var w, z = k(x),
                ...
                B;
            v[15] = y[15] = undefined;
            if (z.length > 16) { z = f(z, x.length * 8) }
            for (w = 0; w < 16; w += 1) { v[w] = z[w] ^ 909522486;
                y[w] = z[w] ^ 1549556828 }
        ...
        function u(y) {
            var A = "0123456789abcdef",
                w = "",
                v, z;
            for (z = 0; z < y.length; z += 1) { v = y.charCodeAt(z);
        ...
        h.md5 = function(w, x, v) {
            ...
            if (!v) {
                return s(x, w) } else {
                return i(x, w) } } }(typeof jQuery === "function" ? jQuery : this));
    a.encodeBase64 = function(k) {
        ...
        var g = 0,
            c = k.length,
            e = "";
        while (g < c) { j = k.charCodeAt(g++) & 255;
            if (g == c) { e += d.charAt(j >> 2);
                e += d.charAt((j & 3) << 4);
                e += "==";
                break }
            h = k.charCodeAt(g++);
    ...
    a.initEdit = function(d) {
        ...
        function q(t, z) {
            ...
            w = { name: t, css: v.css };
            if (v.path) {
                if (a.getType(v.path[0]) === "array") { w.path = [];
                    for (var s = 0; s < v.path.length; s++) { w.path[s] = a.getSrc(v.path[s][0]) } } else { w.path = a.getSrc(v.path[0]) } }
            var x = z ? parseFloat : parseRemToPx;
            var B = { size: ["width", "height"], pos: ["left", "top"] };
            for (var A in B) {
                var y = B[A];
                var u = v[A];
                if (!u) {
                    continue }
                if (a.getType(u) === "array") { w[y[0]] = [], w[y[1]] = [];
                    $.each(u, function(C, D) { w[y[0]].push(x(D[y[0]]));
                        w[y[1]].push(x(D[y[1]])) }) } else { w[y[0]] = x(u[y[0]]);
                    w[y[1]] = x(u[y[1]]) } }
            c[A] = w;
            return w }
        ...
        function i(w, t, v) { $$("#uploadBg").fadeIn(200);
            ...
            return false }
        if (_manage) { d.setAllEdit = function() { e();
                hg.edit.hasInitEle = true;
                $.each(k, function(s, t) {
                    if (t.css) { t.crrCssArg = $.extend(true, [], t.css, o[s].css) } });
                $.each(o, function(t, s) { m([s, k[t]]) }) } } else { e() }

        function e() { $.each(o, function(v, t) {
                var w = k[v],
                    u = $(".editTarget-" + t.name),
                    x = t.pos,
                    A = t.size,
                    z = t.path;
                var s = function(B) { B.each(function(E) {
                        var D = n(x, E),
                            C = n(A, E);
                        if (D && D.top && D.left) { $(this).css({ top: D.top, left: D.left }).addClass("abs") }
                        if (C && C.width && C.height) { $(this).css({ width: C.width, height: C.height }) } }) };
                s(u);
                if (z) {
                ...
                if (t.css) { $.extend(true, w.css, t.css);
                    $.each(w.css, function(B, C) {
                        if (!C.from) { C.from = ".editTarget-" + t.name + ",.editRelate-" + t.name }
                        if (C.css) { $.each(C.css, function(E, D) {
                                if (!D.from) { D.from = C.from }
                                l($(D.from), D, D.opt === 0) }) } else { l($(C.from), C, C.opt === 0) } }) }
    ...
    a.initSound = function(e, f, d) {
        if (d) { $.each(d, function(i, j) { f[i].path = j.path;
                if (g_config.isModel) { e[i].optFlag = f[i].optFlag } }) }
        if (e) { $.each(e, function(j, k) {
                var i = f[j];
        ...
        $(function() { $.setSoundM() });
        var g = { list: e, listDef: f, allowPlay: true, setPlayPower: function(j, k) {
                if (typeof j == "boolean") { this.allowPlay = j }
                var i = c[j];
                if (!i) { a.tlog("Sound_setPlayPower_err", "这个key:" + j + " 不存在!");
                    return this }
                i._allowPlay = k }, play: function(m, o, k) {
                if (_manage) {
                    return }
                if (!this.allowPlay) {
                    return this }
        ...
        return g };
    a.initCallBack = function(d, c) { d = d || {};
        var e = {};
    ...
    a.initAreaLimit = function(e) {
        if (!_manage && e) {
            ...
            var f = setInterval(function() {
                if (d >= c) { clearInterval(f);
                    return }
                d++;
                if (!g_config.ipInfo || !g_config.ipInfo.provice) { $.ajax({ type: "post", url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=getIPInfo", success: function(h) {
                            var g = $.parseJSON(h);
                            if (g.success) { g_config.ipInfo = g.ipInfo } } }) } else { clearInterval(f);
                    return } }, 1500) } };
    a.getAreaByWx = function(f) {
        if (g_config.test) { g_config.ipInfo.provice = "广东";
            g_config.ipInfo.city = "广州";
            f && f() } else {
            try { wx.ready(function() {
                    if (g_config.isOpenAreaLimit) { wx.getLocation({ type: "wgs84", success: function(e) {
                                var h = e.latitude;
                                var g = e.longitude;
                                c(h, g, f) }, fail: function(e) { a.statusMsg("当前微信版本不支持定位或没开启定位服务，请联系活动主办单位", "") }, cancel: function() { a.statusMsg("用户拒绝了授权地理位置信息", "") } }) } }) } catch (d) { f && f() } }

        function c(i, e, j) {
            var h = new qq.maps.Geocoder();
            var g = new qq.maps.LatLng(i, e);
            h.getAddress(g);
            h.setComplete(function(l) {
                if (l) {
                    var n = l.detail.addressComponents.city;
                    var k = l.detail.addressComponents.province;
                    var m = l.detail.addressComponents.country;
                    g_config.ipInfo.contry = m;
                    g_config.ipInfo.provice = k;
                    g_config.ipInfo.city = n;
                    j && j() } });
            h.setError(function() { j() }) } };
    a.initWxConfig = function(k) { a.wxConfigArg = { url: k.fullUrl };
        i({ haveAward: g_config.haveAward, plInfo: k.plInfo, bestRankInfo: g_config.bestRankInfo, comfortAwardStyle: g_config.comfort.cas });
        wx.config({ debug: false, appId: k.jsSdkAppid, timestamp: k.timestamp, nonceStr: k.nonce_str, signature: k.signature, jsApiList: ["checkJsApi", "onMenuShareTimeline", "onMenuShareAppMessage", "onMenuShareQQ", "onMenuShareWeibo", "hideMenuItems", "showMenuItems", "hideAllNonBaseMenuItem", "showAllNonBaseMenuItem", "translateVoice", "startRecord", "stopRecord", "onRecordEnd", "playVoice", "pauseVoice", "stopVoice", "uploadVoice", "downloadVoice", "chooseImage", "previewImage", "uploadImage", "downloadImage", "getNetworkType", "openLocation", "getLocation", "hideOptionMenu", "showOptionMenu", "closeWindow", "scanQRCode", "chooseWXPay", "openProductSpecificView", "addCard", "chooseCard", "openCard"] });
        wx.ready(function() { $("#awardCollectionBtn").on("touchstart", function() {
                var r = true;
                var o = g_config.wxAward;
                var m = o.genewxcard;
                if (m) {
                    if (g_config.status == 0) { a.statusMsg("活动尚未发布", "无法加入到微信卡券");
                        return }
                    var n = o.wxcardid;
                    ...
                    a.log(n);
                    a.log(l);
                    $.ajax({ type: "post", url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=getCardSign&cardTick=" + k.cardTicket + "&cardId=" + n + "&code=" + l + "&openId=" + g_config.openId, error: function() {
                            if (m_debug) { alert("服务繁忙，请稍候重试") } }, success: function(t) { a.tlog("getCardSign", t);
                            var s = $.parseJSON(t);
                            var u = '{"timestamp":"' + s.timestamp + '","openid":"' + g_config.openId + '","nonce_str":"' + s.nonce_str + '","code":"' + l + '","signature":"' + s.sign + '"}';
                            if (p && p < Date.parse(new Date())) { r = false }
                            a.log("depositTime=" + p);
                            if (r) { wx.addCard({ cardList: [{ cardId: n, cardExt: u }], success: function(w) { a.tlog("addCard", w);
                                        a.logDog(1000108, 1);
                                        var v = w.cardList[0];
                                        r = false;
                                        $("#awardCollectionBtn").text("打开微信卡券");
                                        if (v.isSuccess) { $.ajax({ type: "get", url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=setCode&code=" + q + "&gameId=" + g_config.gameId, error: function() {
                                                    if (m_debug) { alert("服务繁忙，请稍候重试") } }, success: function(y) { a.tlog("setCode", y);
                                                    var x = $.parseJSON(y) } }) } } }) } else { wx.openCard({ cardList: [{ cardId: n, code: l }] }) } } }) } }) });
        wx.ready(function() {
            if (k.isHideMenuItems) { wx.hideMenuItems({ menuList: ["menuItem:copyUrl"] }) } else { wx.showAllNonBaseMenuItem() }
            try {
                if (k.isHideShareBtn) { wx.hideMenuItems({ menuList: ["menuItem:share:timeline"] }) }
                wx.error(function(m) {
                    if (m_debug) { alert($.toJSON(m)) } }) } catch (l) { alert(l.message) } });
        wx.ready(function() {
            ...
            if (l) { n = "apple" } else {
                if (m) { n = "android" } }
            if (!g_config.isFromZhuliShare && isPublish) { wx.getNetworkType({ success: function(o) {
                        var p = o.networkType;
                        var q = "other";
                        if (p === "wifi") { q = "wifi" } else { q = "other" }
                        $.ajax({ url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=setBehavior&gameId=" + g_config.gameId + "&openId=" + g_config.openId, type: "post", data: { netType: q, sysType: n }, success: function(r) {} }) } }) } });

        function j(s, p, r) {
            if (!_manage) { s = s.replace("红包", "") }
            s = a.decodeHtml(s);
            ...
            if (!k.dynamicShareUrlRootEmpty) {
                ...
                for (var q = 0; q < m.length; q++) {
                    ...
                    if (o.length > 0) { o += "&" }
                    o += l }
                if (m_debug) { n = k.dynamicShareUrlRoot + "share.jsp?fsl=" + k.fsl + "&aid=" + g_config.aid + "&id=" + g_config.gameId + "&" + o } else { n = k.dynamicShareUrlRoot + g_config.aid + "/" + g_config.gameId + "/share.jsp?fsl=" + k.fsl + "&" + o } }
            ...
            if (k.openStrongAttention) { p = a.setUrlArg(p, ["isOfficialLianjie", "false"]);
            ...
            wx.ready(function() {
                var u = k.shareImg;
                if (!/^http:/.test(u) && /^\/\//.test(u)) { u = "http:" + u }
                try { wx.onMenuShareAppMessage({ title: a.decodeHtml(k.activeName), desc: s, link: p, imgUrl: u, type: "link", success: function() { $.ajax({ type: "post", url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=setShareNum&aid=" + g_config.aid + "&gameId=" + g_config.gameId + "&openId=" + g_config.openId + "&type=friend&shareDeep=" + g_config.shareDeep, error: function() {
                                    if (m_debug) { alert("服务繁忙，请稍候重试") } }, success: function(v) {
                                    if (!m_debug) { a.logDog(1000004, g_config.srcId);
                                        a.logObjDog(1000092, 2, g_config.gameId);
                                        a.logDog(1000055, 0) }
                                    g(v);
                                    r && r(v) } }) }, cancel: function() {} });
                    wx.onMenuShareTimeline({ title: s, link: n, imgUrl: u + "xxxxx", success: function(v) { $.ajax({ type: "post", url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=setShareNum&aid=" + g_config.aid + "&gameId=" + g_config.gameId + "&openId=" + g_config.openId + "&type=pyq&shareDeep=" + g_config.shareDeep, error: function() {
                                    if (m_debug) { alert("服务繁忙，请稍候重试") } }, success: function(w) {
                                    if (!m_debug) { a.logDog(1000004, g_config.srcId);
                                        a.logObjDog(1000092, 2, g_config.gameId);
                                        a.logDog(1000055, 1) }
                                    g(w);
                                    r && r(w) } }) }, cancel: function(v) {} });
                    a.tlog("当前分享朋友链接：", p);
                    a.tlog("当前分享朋友圈链接：", n) } catch (t) { alert(t.message) } });
        ...
        function e(l) { l = l.replace(/\u200b/g, "").replace(/&nbsp;/g, " ").replace(/&lt;/g, "<").replace(/&gt;/g, ">");
            return l.replace(/<span class=["']tag["'] contenteditable=["']?false["']?>(.+?)<\/span>/g, function(n, m) {
                switch (m) {
                    case "玩家名称":
                        if (g_config.style == 48) {
                            return a.hykjUserName } else {
                            if (a.captainUserName) {
                                return a.captainUserName } else {
                                return g_config.userName } }
                    case "游戏成绩":
                        if (g_config.style == 49) {
                            return parseInt(a.currentScore) + g_config.scoreUnit } else {
        ...
        function c(l, m) {
            ...
            if (g_config.drawType != 3) {
                if (g_config.style == 49) { j(e(k.wxShareText_def), l, m);
                    return }
                if (a.currentAward) { j(e(k.wxShareText_award), l, m) } else {
                    if (a.currentRank) { j(e(k.wxShareText_rank), l, m) } else { j(e(k.wxShareText_def), l, m) } } } }

        function g(n) {
            var l = $.parseJSON(n);
            a.tlog("shareaward", l.shareaward);
            if (l && l.shareaward) { drawTimesLimit += k.addDrawTime;
                if (g_config.style != 50) { drawTotalLimit += k.addDrawTime }
                if (g_config.drawType != 0 && a.resulePoup.resuleArg) { a.resulePoup.resuleArg.count += k.addDrawTime;
        ...
        function i(n) {
            ...
            if (q) {
                ...
                var o = m.awardLevel;
                if (o != 900 && o > 0) {
                    if (k.nameList && k.nameList.length >= o) { a.currentAwardStyle = k.nameList[o - 1] } else { a.currentAwardStyle = l[o - 1] } } else { a.currentAwardStyle = n.comfortAwardStyle } }
        ...
        d.setWxShareArg = i;
        a.wxConfig = d };
    a.addJoinGameBehavior = function() { $.ajax({ url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=joinGameBehavior", type: "post", data: { gameId: g_config.gameId, openId: g_config.openId, shareDeep: g_config.shareDeep }, success: function(c) {} }) };
    a.setGameEnd = function() {
        ...
        $(".home").css({ height: c, background: "#424242", width: "100%", position: "fixed", "z-index": "500" });
    ...
    a.showRuleDecide = function() {
        ...
        if (!a.getLocalStorage(c)) { a.setLocalStorage(c, "-");
            showRule() } };
    a.initJsFoot = function(d) { $(function() { a.sortRuleBox.init(d.rulesortstr);
            a.unPublishMsg();
            if (isPublish) { $(".unPublish").css("top", -2 + "rem") }
            var k = a.decodeHtml(d.hostName);
            var g = a.decodeHtml(d.hostLink).replace("http://", "");
            var i = a.decodeHtml(d.menuLink).replace("http://", "");
            var f = $(".menuName");
            if (d.fromFav && g_config.haveAward) { $("#resule-gift-sucImg").data("openCode", d.fromFav);
                showAwardDetail4Draw("fromFav") }
            if (!_manage && d.openAwardExp) {
                if (!(g_config.style == 55)) { a.showRuleDecide() } }
            $("#showRankNum").text(rankShowNum);
            $(".hostName").text(k);
            $(".resule-status-send").on("touchstart", function() {
                if (_manage) {
                    return }
                if (g_config.haveAward) { a.changePoup(3) } else { luckDraw() } });
            $("#helpGuideBox").on("touchend", function() { $("#helpGuideBox").hide() });
            $("#awardInfo").on("click", ".codeInfoBox", function(l) {
                if (_manage) {
                    return }
                a.openAwardDetail.call(this, l) });
            $("#resule-gift-sucImg, .clickHere").on("touchstart", function(l) { showAwardDetail4Draw();
                $("#clickHere").remove() });
            $(".awardCloseIcon").on("touchstart", function() { a.wxConfig.removeWxShareUrlArg("fromFav");
                $("#awardDetailBox").hide();
                $("#resule-gift-box,resule-status-box").hide();
                $(".gameBox").add(".home").removeClass("overflow-y-hidden");
                g_config.showSkillSup && $(".bottomSkill").show() });
            $(".codeImg").on("click", function() { $(".codeDetailImgBox").show() });
            $(".codeDetailImgBox").click(function() { $(this).hide() });
            $(".awardCusText").on("touchstart", function() {
                if (g_config.afterWxCard) { $("#ticketDetailBox").show() } else {
                    var l = $("#awardCusTextInfo");
                    if (l.css("display") == "block") { $(this).find(".awardDeailIcon").removeClass("awardIncoSlideDown");
                        $(this).find(".awardDeailIcon").addClass("awardIncoSlideUp") } else { $(this).find(".awardDeailIcon").removeClass("awardIncoSlideUp");
                        $(this).find(".awardDeailIcon").addClass("awardIncoSlideDown") }
                    l.slideToggle() } });
            $(".ticketitle .closeBtn").on("touchstart", function() { $("#ticketDetailBox").hide() });
            if (d.isHideFxts) { $(".resule-fxts").hide() }
            g_config.style != 9 && g_config.showSkillSup && a.logDog(1000028, 0);
            if (g_config.aid == 7171826 && g_config.style != 9 && g_config.showSkillSup) { a.logDog(1000062, 2) } else {
                if (g_config.aid == 8566249 && g_config.style != 9 && g_config.showSkillSup) { a.logDog(1000067, 2) } }
            if (g_config.aid == 7171826 || g_config.aid == 8566249) { $(".home,.showBottomSkillPage").append('<div class="bottomSkill skillInfo">本游戏由 <span class="yySkill" style="text-decoration:underline;">凡科互动</span> 提供</div>');
                $(".yySkill").click(function() {
                    if (g_config.aid == 7171826) { a.logDog(1000062, 3) } else {
                        if (g_config.aid == 8566249) { a.logDog(1000067, 3) } }
                    $("#yyAttentionMask").show() }) } else {
                if (g_config.status == 3) { $(".home,.showBottomSkillPage").append('<div class="bottomSkill skillInfo">页面技术由 <a href=' + d.supportUrl + ">" + d.skillSupport + "</a> 提供</div>") } else {
                    if (g_config.style != 46) {
                        if (g_config.style == 55) { $(".gameBox,.showBottomSkillPage").append('<div class="bottomSkill skillInfo">页面技术由 ' + d.skillSupport + " 提供</div>") } else { $(".home,.showBottomSkillPage").append('<div class="bottomSkill skillInfo">页面技术由 ' + d.skillSupport + " 提供</div>") } } } }
            _manage && $("a").on("click", function() {
                return false });
            $(".homePoupMask").on("touchend", function(l) { l.preventDefault();
                l.stopPropagation();
                if (_manage) {
                    return }
                if ($(l.target).hasClass("homePoupMask")) { $(this).hide() } });
            $(".menuBtnBox").on("touchend", function() { a.logDog(1000012) });
            if (d.showJoinNum) { $("#joinNumLine").show() } else { $("#joinNumLine").hide() }
            if (d.showRedDot) { $("#Award_Round_Dot").css("display", "inline-block") }
            if (d.showMDRedDot) { $("#Mingdan_Round_Dot").css("display", "inline-block") }
            if (d.showAwardBtn) { $("#myAwardBtn").show() }
            if (d.isSelAwardLine) { $("#selfAwardLine").show();
                $("#awardLineBox").hide() } else { $("#awardLineBox").show();
                $("#selfAwardLine").hide() }
            if (!d.showSkillSup) { $(".skillInfo").hide();
                $(".skillInfo").addClass("hideSkill") }
            if (d.menuStyle == 1) { $(".menuBtnBox").hide() } else {
                if (d.menuStyle == 2) {
                    if ($.trim(i).length == 0) { f.css("text-decoration", "none") } else { f.attr("href", a.decodeHtml(d.menuLink)) } } else {
                    if (d.menuStyle == 3) {
                        (function() {
                            var l = $("#attentionMask");
                            if (a.decodeHtml(d.menuLink) === "") { $("#QRImg").remove() }
                            $(".menuBtnBox").on("touchend", function() { l.show() }) })() } } }
            if ($.trim(g).length == 0) { $(".hostName").css("text-decoration", "none");
                $("a.hostName").attr("href", "#") }
            if (!isLimitDraw) { $(".totalDraw").hide() } else { $(".totalDraw").show();
                $(".dayDraw").hide();
                if ((drawTimesLimit - count) < (drawTotalLimit - totalCount)) { $(".dayDraw4Total").show() } }
            if ($("#awardExplain").text().length == 0) { $("#awardExplain_h").hide() }
            $("a.hostName").click(function() { a.logDog(1000013) });
            if (skillSupportType == 1) { $(".skillLine").css("padding", "0");
                $(".skillCont").hide() } else {
                if (skillSupportType == 3) { $(".skillName").text(skillName);
                    $(".skillName").attr("href", skillLink) } }
            if (skillSupportType != 3 || skillLink.indexOf("mp.weixin.qq.com/s?__biz=MjM5MTk5MjI3OA==&mid=209854000&idx=1&sn=82241d924839270d3ea820ad2d56c01b#wechat_redirect") >= 0) { $(".skillName").click(function() {
                    if (_manage) {
                        return }
                    a.logDog(1000013);
                    if (g_config.authVer == HdVerDef.FREE) { a.logDog(1000070, 1) } else {
                        if (g_config.authVer == HdVerDef.BJ) { a.logDog(1000070, 3) } else {
                            if (g_config.authVer == HdVerDef.BY) { a.logDog(1000070, 5) } } }
                    $("#skillSupMask").show();
                    return false }) }
            $(".skillInfo a").click(function() {
                if (_manage) {
                    return }
                a.logDog(1000005, 0);
                if (g_config.authVer == HdVerDef.FREE) { a.logDog(1000070, 0) } else {
                    if (g_config.authVer == HdVerDef.BJ) { a.logDog(1000070, 2) } else {
                        if (g_config.authVer == HdVerDef.BY) { a.logDog(1000070, 4) } } }
                typeof g_config.isAOpenId == "number" && a.logDog(1000115, 3 + g_config.isAOpenId);
                if (g_config.aid == 7171826 || g_config.aid == 8566249) {
                    if (g_config.aid == 7171826) { a.logDog(1000062, 1) } else {
                        if (g_config.aid == 8566249) { a.logDog(1000067, 1) } }
                    $("#yyAttentionMask").show();
                    return false } else {
                    if (!g_config.isOem) { $("#skillSupMask").show();
                        return false } } });
            if (hg.sound.list && hg.sound.list.length > 0) { $(".home").add(".gameBgBox").append('<div class="' + (_manage ? "" : "soundIconOff ") + "soundIcon" + (hg.sound.list[0].optFlag != 1 ? "" : " soundIconHide") + '" style="z-index:90"></div>');
                $('.home .soundIcon:not(".soundIconPlay")').css({ left: d.soundIcon_l, top: d.soundIcon_t });
                if (d.drawType == 1 || g_config.style == 4) { $(".home .soundIcon").hide() } }
            if (!_manage) { $(".gameBgBox .soundIcon").css("z-index", "100");
                $(".soundIcon").addClass("soundIconPlay");
                var j = "soundPause|" + g_config.aid + "|" + g_config.gameId + "|" + g_config.openId;
                $(".soundIcon").on("touchstart", function() { event.stopPropagation();
                    event.preventDefault();
                    var l = $(".soundIcon");
                    if (!l.hasClass("soundIconOff")) { hg.sound.allowPlay = false;
                        hg.sound.pauseAll();
                        a.setLocalStorage(j, "-") } else { hg.sound.allowPlay = true;
                        hg.sound.readyPlay(0, 0, "loop");
                        a.setLocalStorage(j, "") } });
                if (a.getLocalStorage(j)) { hg.sound.allowPlay = false;
                    hg.sound.pauseAll() }
                $("#ruleImg").addClass("ruleImgAnimate");
                $("#startBtnImg,.startBtnImg").on("touchend", function(l) {
                    if (!startBtnAjax.call(this, l)) {
                        return }
                    hg.sound.play("startButton");
                    if (typeof hg.sound.cache[0] !== "undefined" && typeof hg.sound.cache[0].playing !== "undefined" && !hg.sound.cache[0].playing && g_config.style != 51 && g_config.style != 9 && g_config.style != 48) { hg.sound.readyPlay(0, 0, "loop") } });
                hg.assets.onload(startBtnDelay) } else { a.initModuleLayer([
                    ...
                    ["#resule-gift-sucImg", -1, "", [{ from: "#resule-gift-sucImg", title: "奖品图片", limit: "5000k", defSrc: d.awardImage }]],
                    ["#titleImg", -1, "", [{ from: "#titleImg", title: "标题图片", limit: "5000k", defSrc: d.titleImg_path_def }]],
                    ["#startBtnImg", -1, "", [{ from: "#startBtnImg", title: "按钮图片", limit: "5000k", defSrc: d.startImg_path_def }]],
                    ["#ruleImg", -1, "", [{ from: "#ruleImg", title: "活动奖品", limit: "5000k", defSrc: d.awardIconImgPath_def }]]
                ]);
                if (!g_config.afterWxCard) { a.initModuleLayer([
                        ["#awardDetailBox .awardCusText", 1, "awardCusText"],
                        [".codeImg", 1, "codeImg"],
                    ]) }
                $(".titleImg").addClass("abs");
                a.moduleSlaveImgResize([
                    [$("#titleImg"), { width: d.titleImg_w_def, height: d.titleImg_h_def }],
                    [$("#startBtnImg"), { width: d.startBtn_w_def, height: d.startBtn_h_def }],
                    [$("#logoImg"), { width: d.logoImg_w_def, height: d.logoImg_h_def }, "#limitRange"],
                ]);
                a.moduleDraggale([
                    ["#titleImg", true],
                    ["#startBtnImg", true],
                ...
                function h(m, l) { m.css("z-index", l) }
                $("#theGetPricePic,#theGetPricePicTwo,#theGetPricePicThree").setResizableOption({ containment: ".cannotGetThePriceBox", }).setDraggableOption({ containment: ".cannotGetThePriceBox", });
                $("#resule-gift-sucImg").setResizableOption({ handles: "s,e,w,se,sw", dxRatio: 2, containment: false, }).addResizableFn("start", function(l, m) { $(this).add($(this).parent()).css({ "max-height": (parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top) / g_rem + "rem" }) }).addResizableFn("resize", function(m, n) { $(this).add($(this).parent()).css({ left: 0, top: 0, });
                    var l = parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top;
                    if ($(this).height() > l) { $(this).add($(this).parent()).css({ "max-height": l / g_rem + "rem" }) } });
                a.moduleSlaveImgResize($("#resule-gift-sucImg"), { width: d.awardImg_w_def, height: d.awardImg_h_def });
                $("#resule-gift-sucImg").add($("#resule-gift-sucImg").parent()).css({ "margin-left": "auto", "margin-top": "1.6rem", "margin-right": "auto", "margin-bottom": "1.6rem", });
                ...
                (function() {
                    if (hg.sound.list && hg.sound.list.length > 0) {
                        ...
                        if (g_config.style != 0 && g_config.style != 1 && g_config.style != 2 && g_config.style != 24) { l = ".gameBgBox" }
                        if (g_config.style == 34) { l = "#gameTopBar,#gameLayerBox1,#gameLayerBox2,#gameLayerBox3" }
                        if (g_config.style == 45) { l = ".gameInfoBoxMain" }
                        if (g_config.style == 48) { l = "#gameContent,.friendEnterBoxer" }
                        if (g_config.style == 9) { l = "#openBox" }
                        if (g_config.style == 54) { l = ".home" }
                        parent.Edit.addEditBtn("编辑音效", l, function() { parent.Edit.showEditByGame(-4, "", []);
                            return false }) }
                    g_config.style != 49 && parent.Edit.addEditBtn("编辑背景", "#ruleImg,.home,.gameBgBox,.gameBgBox2", function() {
                        var n = [];
                        var q = {};
                        if ($(".home").is(":visible")) { q.from = "#homeBg";
                            q.defSrc = d.homeBgPath_def } else {
                            if ($(".gameBgBox2").is(":visible")) { q.from = "#gameBg2" || ".gameBgBox2";
                                q.defSrc = d.gameBg2Path_def } else {
                                if ($(".gameBgBox").is(":visible")) { q.from = "#gameBg";
                                    q.defSrc = d.gameBgPath_def } } }
                        if (hg.edit && hg.edit.bgList) {
                            var p, o;
                            for (var m = hg.edit.bgList.length - 1; m >= 0; m--) { p = hg.edit.bgList[m];
                                o = $(p.from);
                                if (o.length > 0 && o.eq(0).is(":visible")) { q = p;
                                    break } } }
                        q.title = "背景图片";
                        q.size = "640px*1136px";
                        q.limit = "5000k";
        ...
        function c() { $(".one").hide().siblings().show();
        ...
        function e() { $("#resule-status-box").hide();
        ...
        $(function() { $(".one").show();
            ...
            if (g_config.drawType == 1 && !_manage && otherPlayerInfo && typeof otherPlayerInfo.headImg !== "undefined" && typeof otherPlayerInfo.name !== "undefined" && typeof otherPlayerInfo.score !== "undefined" && typeof otherPlayerInfo.rank !== "undefined" && typeof otherPlayerInfo.award !== "undefined" && false) {
                ...
                if (!a.getLocalStorage(f)) {
                    if (otherPlayerInfo.award != "") { $(".WhetherAward").text("获得" + otherPlayerInfo.award).show() }
                    ...
                    c();
                    a.setLocalStorage(f, "-") } }
            $(".winAWard a").on("touchend", function() { showRule() });
            $(".resule-status-rightNow").on("touchend", function() { hg.on("startGame", function() { e();
                    hg.off("startGame", arguments.callee) });
                $("#startBtnImg").trigger("touchstart");
                $("#startBtnImg").trigger("touchend") });
            $(".resule-status-firstPage").on("touchend", function() { e() });

            function i(j) {
                return function(n, m) {
                    var l, k;
                    if (typeof n === "object" && typeof m === "object" && n && m) { l = n[j];
                        k = m[j];
                        if (l === k) {
                            return 0 }
            ...
            if (_manage) { g.append('<div class="showAwardBox"><ul class="awardInfoList"><li><div class="uerItem"><img src="' + g_config.headImg + '"><span class="winner">范女神<span>获得了<span class="award">100元优惠券</span></div><div class="uerItem"><img src="' + d.headImg2 + '"><span class="winner">小星儿<span>获得了<span class="award">50元优惠券</span></div><div class="uerItem"><img src="' + d.headImg3 + '"><span class="winner">LVYD<span>获得了<span class="award">30元优惠券</span></div><div class="uerItem"><img src="' + d.headImg4 + '"><span class="winner">萌妹子<span>获得了<span class="award">10元优惠券</span></div></li></ul></div>');
                $("#skillLine").css("min-height", "1rem") } else {
                if (!_manage && playerAwardList.length > 3) { g.append('<div class="showAwardBox"><ul class="awardInfoList"><li></li></ul></div>');
                    var h = "";
                    $.each(playerAwardList, function(j, k) { h += '<div class="uerItem"><img src="' + JSON.parse(this.info).headImg + '"><span class="winner">' + this.name + '<span>获得了<span class="award">' + this.award + "</span></div>" });
                    if (g_config.style == 55) { $(".showAwardBox").hide();
                        setTimeout(function() { $(".awardInfoList li").append(h);
                            $(".showAwardBox").awardSlide({ padding_right: "0.8rem" }) }, 3500) } else { setTimeout(function() { $(".awardInfoList li").append(h);
                            $(".showAwardBox").awardSlide({ padding_right: "0.8rem" }) }, 100) }
                    $(".homeBtnBox").css("bottom", "3rem") } }
            if (!_manage) {
                if (d.dangerType == 0) {
                    if (g_config.status == 3) { a.setGameEnd() } } }
            setTimeout(function() {
                var l = $(window).height() / $(window).width() < 1.575;
                var j = l ? $(window).width() * 1.575 : $(window).height();
                var k = g_config.showSkillSup ? 1 : 0;
                if (_manage) { $(".showAwardBox").css("top", (j - $(".bottomSkill").outerHeight() * k - $(".showAwardBox").outerHeight() - 2) / g_rem + "rem") } else { $(".showAwardBox").css("top", (j - $(".bottomSkill").outerHeight() * k - $(".showAwardBox").outerHeight()) / g_rem + "rem") }
                if (g_config.showSkillSup) { $(".showAwardBox").css("top", (j - $(".bottomSkill").outerHeight() * k - $(".showAwardBox").outerHeight() - 2) / g_rem + "rem") } }, 50);
            if (!g_config.showSlide) { $(".showAwardBox").hide() } else {
                if (!_manage) { $(".showAwardBox").addClass("footerBox") } } }) };
    a.setStartBtnHeight = function(g, e) {
        var o = $(window).height();
        if (o === 0) { console.log("window高度为零！");
            setTimeout(function() { a.setStartBtnHeight(g, e) }, 20);
            return }
        ...
        if (p.length > 0 && c > o - d) { p.css("top", (o - d - p.outerHeight() - 5) / g_rem + "rem") }
        if (m.length > 0 && j > o - d) { m.css("top", (o - d - m.outerHeight() - 5) / g_rem + "rem") }
        if (n.length > 0 && l > o - d) { n.css("top", (o - d - n.outerHeight() - 5) / g_rem + "rem") }
        if (r.length > 0 && k > o - d) { r.css("top", (o - d - r.outerHeight() - 5) / g_rem + "rem") }
        if (s.length > 0) { s.css("bottom", (d + 8) / g_rem + "rem") }
        if (q.length > 0 && i > o - d) { q.css("top", (o - d - q.outerHeight() - 5) / g_rem + "rem") }
    ...
    a.getCookie = function(c) {
        if (document.cookie.length > 0) { c_start = document.cookie.indexOf(c + "=");
            if (c_start != -1) { c_start = c_start + c.length + 1;
    ...
    a.removeLocalStorage = function(d, c) {
        if (!localStorage) {
            if (c) { a.setCookie(d, "", -1) }
            a.log("不支持localStorage");
            return }
        localStorage.removeItem(d) };
    a.refresh = function() { window.location.reload() };
    a.noLoadingRefresh = function() { window.location.href = document.URL.indexOf("noLoading") >= 0 ? document.URL : (document.URL + (document.URL.indexOf("?") >= 0 ? "&" : "?") + "noLoading=fff") };
    a.isIPhone = function() {
    ...
    a.changeTimeLimit = function(c) {
        ...
        for (var e = 0; e < f.length; e++) {
            var h = e,
                d = e;
            while (f[d + 1] == (f[d] + 1)) { d++ }
            if (h == d) { g = g + (g ? "、" : "") + b(f[h]) } else { g = g + (g ? "、" : "") + b(f[h]) + "至" + b(f[d]) }
            e = d }
        return g };
    ...
    a.setUrlArg = function() {
        ...
        for (var e = 1; e < g.length; e++) {
            var f = g[e];
            if (a.getType(f) === "array") { c.push(f[0]);
                if (e > 1) { h += "&" }
                h += (f[0] + "=" + f[1]) } }
    ...
    a.shuffle = function(e) {
        ...
        for (var g = 0; g < d - 1; g++) {
            ...
            e[d - g - 1] = f }
        return e };
    (function() { a.logStd = c(40, function(d, g) {
            try { g = g.slice(0, 2000) } catch (f) {}
            $.ajax({ type: "post", url: g_config.ajaxUrl + "logJsErr_h.jsp?cmd=jsLogStd", data: { msg: "hgLog-" + d + " msg=" + g + " aid=" + g_config.aid + " gameId=" + g_config.gameId + " openId=" + g_config.openId }, }) });
        a.logErr = c(3, function(d, e) { $.ajax({ type: "post", url: g_config.ajaxUrl + "logJsErr_h.jsp?cmd=jsLogErr", data: { msg: "hgErr-" + d + " msg=" + e + " aid=" + g_config.aid + " gameId=" + g_config.gameId + " openId=" + g_config.openId }, }) });

        function c(d, e) {
            return function(g, h, f) {
                if (typeof e[g] == "undefined") { e[g] = 0 } else { e[g]++ }
            ...
            if (!d) { d = setTimeout(function() {
                    if (c.length == 1) { 
                        ...
                        c = c.map(function(g, h) {
                            var i = { id: g[0], src: g[1], };
                            if (!isNaN(g[2])) { i.objId = g[2] }
                            return i 
                        });
                    /*    a.logDogList(c) */
                    }
                    d = null;
                    c = [] }, 0) } } })();
    a.logObjDog = function(e, c, d) { a.logDog(e, c, d) };
    /*a.logDogList = function(c) { 
        $.ajax({ 
            type: "post", 
            url: g_config.ajaxUrl + "log_h.jsp?cmd=dogList", 
            data: { dogList: $.toJSON(c) }, 
        }) 
    };*/
   /* a.logDogSynch = function(f, d, e) {
        var c = isNaN(e) ? "" : "&objId=" + a.encodeUrl(e);
        $.ajax({ 
            type: "get", 
            url: g_config.ajaxUrl + "log_h.jsp?cmd=dog&dogId=" + a.encodeUrl(f) + "&dogSrc=" + a.encodeUrl(d) + c, 
        }) 
    };*/
    a.bulletparam = { wxheadimg: true, loop: true, gutter: 34, autoplay: true, isScroll: true };
    a.initComment = function(c) { $.extend(a.bulletparam, c) } })(HdGame);
HdGame.poupValidate = function(b) {
    if (!$("#spxdPage").hasClass("imp-hide")) { $("#spxdPage").addClass("imp-hide") }
    if (window.top.$("#valiteInputIframe").length == 0) {
        var a = g_config.ajaxUrl.replace("/ajax/", "");
        $("#validteBoxerBg").data("_theCallBack", b).show();
        $("#validteBoxerBg").append('<iframe id="valiteInputIframe" frameborder="0" scrolling="no" src="' + a + '/poupvalite.jsp" style="height: 25rem; width: 100%;"></iframe>') } else { $("#validteBoxerBg").data("_theCallBack", b).show() } };
var CUtil = (function() {
    var c = function(e, d) {
        if (d == null) { d = e;
            e = 0 }
...
var Comment = (function() {
    ...
    function b(f, d) {
        ...
        e += "</div>";
        if (a.loop) { e += '<div class="hdComment">';
            f.forEach(function(i, h) {
                var j = i.headImg ? "background:url(" + i.headImg + ")  no-repeat #fff;background-size: cover;" : "";
                e += '<div class="hdCommentFloat"><div class="hdCommHeadImg"><span class="hdCommPortrait"  style="' + j + '"></span></div><div class="hdCommContent">' + i.content + "</div></div>" });
            e += "</div>" }
    ...
    b.prototype.reset = function(e) {
        ...
        var m = this.$dom.find(".hdCommentFloat"),
            h = 0;
        for (var g = 0, f = m.length; g < f; g++) { h += Math.ceil(m.width()) + parseInt(m.css("margin-right")) + 50 }
        if (h < j) { h = j }
        h = h + (l + i) * 2;
        this.v = (h / 2 + l + i) / this.v;
        var c = this.$dom.find(".hdComment");
        if (a.isScroll) { c.eq(0).addClass("hdCommentAnimate");
            c.css({ "padding-left": l + "px", "padding-right": i + "px", "animation-duration": this.v + "s", "-webkit-animation-duration": this.v + "s" });
            setTimeout(function() { c.eq(1).addClass("hdCommentAnimate") }, (this.v / 2) * 1000) }
        c.css({ width: "50%" });
...
var CView = (function() {
    ...
    function a(d) { this.eachCommListLen = this.eachCommListLen2 = 0;
        this.VIEW_LINE = [1, 2, 3];
        this.comments = [];
        var c = this;
        this.initCommentBase(c, d);
        if (b.autoplay) { $("#hdCommentPanel .replayInput").focus();
            c.$comm_page.hide();
            c.$comm_noshow.hide();
            c.stopAnimation && c.stopAnimation() } else { c.$comm_show.show() } }
    a.prototype.initCommentBase = function(c, e) {
        if (e) { e = $(e) } else { e = $("body") }
    ...
    a.prototype.initData = function(d) {
        var e = document.createDocumentFragment(),
            f, c;
        if (d.bulletList.length > d.VIEW_LINE.length) { d.VIEW_LINE.forEach(function(i, h) { i = d.getSliceData(d);
                f = new Comment(i, h);
                if (!f.$dom) {
                    return }
                c = f.$dom;
                d.comments.push(f);
                e.appendChild(c[0]) }) } else { d.VIEW_LINE.forEach(function(i, h) { i = d.bulletList.splice(0, 1);
                f = new Comment(i, h);
                if (!f.$dom) {
                    return }
                c = f.$dom;
        ...
        d.comments.forEach(function(i, h) { i.reset(h) });
    ...
    a.prototype.getSliceData = function(c) {
        if (!c.bulletList || c.bulletList.length === 0) {
            return }
        if (c.eachCommListLen == 0) { c.eachCommListLen = Math.floor(c.bulletList.length / c.VIEW_LINE.length) ? Math.floor(c.bulletList.length / c.VIEW_LINE.length) : 1;
            c.eachCommListLen2 = c.bulletList.length % c.VIEW_LINE.length }
        var d;
        if (c.eachCommListLen2 > 0) { d = c.bulletList.splice(0, c.eachCommListLen + 1);
            c.eachCommListLen2-- } else { d = c.bulletList.splice(0, c.eachCommListLen) }
        return d };
    a.prototype.getData = function(d, c) { c.$comm_page.show();
        c.bulletList = d };
...
var PlayInfo = (function() {
    ...
    function n() { $(".todayPlayCount").text(e());
        $(".totalPlayCount").text(d());
        $(".playTimesLimit").text(a);
        $(".playTotalLimit").text(b);
        if (k) { $(".dayPlayHint").hide();
            $(".totalPlayHint").show();
            $(".dayPlayHint4Total").show();
            $(".playTotalFont").show() } else { $(".dayPlayHint").show();
            $(".totalPlayHint").hide();
            $(".dayPlayHint4Total").hide();
            $(".playTotalFont").hide() } }
    return { initData: m, getTodayRemainTimes: e, getTotalRemainTimes: d, addPlayTimes: j, addPlayTimesLimit: i, setPlayTimesLimit: f, setPlayTotalLimit: g, setLimitPlay: l } })();
