...
(function(a){ 
        ...
        if (e){ 
            ...
            if (!_manage){ 
                g.cache["0"].data.addEventListener("play", function(){ 
                    $(function() { $(".soundIcon").removeClass("soundIconOff") }) 
                }, false);
                g.cache["0"].data.addEventListener("pause", function() { 
                    $(function() { $(".soundIcon").addClass("soundIconOff") }) 
                }, false) 
            } 
        }
        return g 
    ...
    a.initTime = function(c) {
        ...
        var g = { 
            val: 0, pastTime: 0, interval: 140, range: d, target: null, 
            isDesc: !g_config.countsTimeType, 
            acceList: null, initTime: c, 
            status: "ended", updateFlag: true, 
            targetFlag: true, frameInc: 0, 
            ...
            start: function() {
                ...
                if (
                    this.updateFlag && this.status === "ended") { 
                    this.status = "runing";
                    this.play() 
                } else { 
                    this.status = "runing" 
